...
function setInitData(paraName, pareValue){
	var result=$("ulinkSP").setInitData(paraName, pareValue);
	return result;
}
//批量设置初始化参数,用,,,和:::分隔
function setInitDataAll(spInitParameters){
	var result=$("ulinkSP").setInitDataAll(spInitParameters);
	return result;
}
//获取初始化参数
...
//	if(img1011.disabled==true || isUILocked) return;last_ope_img=img1011;
//}
	...
	if(Keyword=="agentlogin"){
		...
		if(Message=="SessionConnectionStatusChanged"){
			...
			if(ExtenReason=="ENTERAUXWORK"){//未就绪操作成功
				...
				UIforUnlock();if(isDebug)alert("**就绪操作成功!");
			}
			invokeClearWorkItemList();//清除工作列表
		...
		if(Message=="ChannelStateChanged"){
			...
			if(ExtenReason=="MAKECALL"){//外呼失败
				alert("外呼失败,对方忙或无法接通!");
				UIforUnlock();
				UIforBusyEvent();//add by 20120625 for jiayougo
			}
...
//            var Card = getEduValue("Card");//会员卡号
            ...
            if(ExtenReason == _WorkItem_State_WORKING){//应答成功、呼叫成功、取回成功开始工作
            	if(img1004.disabled==false && img1004.name=="answer"){//上次是振铃则本地为应答成功
					...
					//}
            	}else if(last_ope_img==img1004 && img1004.name=="dail"){//外呼接通
...
if(isDemo && isDebug) $("testSPUI").style.display = "block";
$("testSPUI").onchange = function(){
	var evalStr = $("testSPEval").value;
