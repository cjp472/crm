...
QM.ui.TreeCombo = Ext.extend(Ext.form.TriggerField, {
	...
	initList : function() {
		if (!this.list) {
			var cls = 'x-combo-list', listParent = Ext.getDom(this
					.getListParent()
					|| Ext.getBody()), zindex = parseInt(Ext.fly(listParent)
							.getStyle('z-index'), 10);
			if (this.ownerCt && !zindex) {// 找到父容器定义的z-index
				this.findParentBy(function(ct) {
							zindex = parseInt(ct.getPositionEl()
											.getStyle('z-index'), 10);
							return !!zindex;
						});
			}
	...
	restrict : function() {
		...
		wa = Math.max(body.clientWidth, body.offsetWidth, body.scrollWidth), w = Math
				.max(wa, this.minListWidth - wpad);
	...
	setValue : function(node) {// 根据TreeNode的id或者TreeNode对象设置值，显示TreeNode的text属性
		...
		if (typeof (node) == 'string') {
			...
			QM.ui.TreeCombo.superclass.setValue.call(this, node);
			//alert(node.id);
		}
		var text;
	...
	setNameValue : function(node) {// 根据TreeNode的id或者TreeNode对象设置值，显示TreeNode的text属性
		QM.ui.TreeCombo.superclass.setValue.call(this, node);
		this.value = node ? node.id : '';
		return this;
	},
	getHiddenValue : function(node) {
