...
<%
		...
		if(opt != null && !"".equals(opt)){
			...
			filter.addFilter("Q_shortMessage.msgType_SN_EQ", "1");//只取个人消息
			if(opt.equals("_next")){
		...
		try{
			if(inMessage.getShortMessage().getFileid().indexOf(",") >= 0){
				filename = inMessage.getShortMessage().getFilename().split(",");
				fileid = inMessage.getShortMessage().getFileid().split(",");
				for(int i = 0 ; i < fileid.length ; i++){
					file.put(fileid[i],filename[i]);
				}
			}else{
				file.put(inMessage.getShortMessage().getFileid(),inMessage.getShortMessage().getFilename());
			}
		}catch(Exception e){
			file = null;
		}
...
<table  width="98%" cellpadding="0" cellspacing="1">
	<tr>
		<td >
			...
			<input type="hidden" value="${inMessage.shortMessage.sendMsgType}" id="__SENDMSGTYPE"/>
		</td>
	</tr>
	<tr>
		<td style="padding-bottom:10px;">
			...
			&nbsp;&nbsp;
			<!--<font color="green">
				${inMessage.shortMessage.sendMsgType}
				<c:choose>
