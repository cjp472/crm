...
%>
	<table class="table-info" cellpadding="0" cellspacing="1" width="96%">
		...
		<tr>
			...
			<td>
				<c:choose>
					<c:when test="${processForm.status==1}">
						<c:if test="${processForm.transTo!=\"to 驳回\"}"><font color='green'>审批通过</font></c:if>
						<c:if test="${processForm.transTo==\"to 驳回\"}"><font color='red'>驳回</font></c:if>
					</c:when>
