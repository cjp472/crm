...
<%@page import="com.htsoft.oa.model.flow.FormDef"%>
<%@page import="com.ulane.know.service.know.UkKnowTemplateService"%>
<%@page import="com.ulane.know.model.know.UkKnowTemplate"%><html>
	<head>
		<title>在线流程表单设计器</title>
		<%
		String basePath=request.getContextPath();
		//加载表单的设计的数据		
		String knowTmpId=request.getParameter("knowTmpId");
		String projectData="";
		
		if(!"undefined".equals(knowTmpId) && StringUtils.isNotEmpty(knowTmpId)){
			UkKnowTemplateService ukKnowTemplateService =(UkKnowTemplateService)AppUtil.getBean("ukKnowTemplateService");
			UkKnowTemplate ukKnowTemplate =ukKnowTemplateService.get(new Long(knowTmpId));
			if(StringUtils.isNotEmpty(ukKnowTemplate.getTempContent())){
				projectData=ukKnowTemplate.getExtDef();
			}
		}
		%>
		...
		<script type="text/javascript" src="<%=basePath%>/js/formeditor/deploy/ext-complete.js"></script>
		<script type="text/javascript" src="<%=basePath%>/ext3/ext-lang-zh_CN.js"></script>
		<script type="text/javascript" src="<%=basePath%>/js/formeditor/deploy/designer-complete.js"></script>
