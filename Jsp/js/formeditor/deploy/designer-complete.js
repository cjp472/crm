/*
 * Ext GUI Designer Copyright(c) 2010-2012 Ext JS, 优联
 * 
 */
				...
				var getFormItemDef = function(node) {
					if (node.childNodes.length == 0) {
						...
						if (category == '表单控件') {
							...
							formItemDef += Ext.util.JSON
									.encode(node.component.config);
							cnt++;
						}
					} else {
			...
			getJsonConfig : function(a) {
				...
				if (!c) {
					c = Ext.apply({
						...
						id : this.id,
						xcls : this.xcls,
						layoutConfig : this.getLayoutCfg()? userConfig : xds.copy(this.userConfig)
						}, this.defaultConfig);
	...
	constructor : function() {
		...
		xds.Inspector.superclass.constructor.call(this, {
			...
			keys : [{
				...
				fn : function() {
					if (xds.active) {
						Ext.Msg.confirm("删除组件", "您确认要永久删除这个组件吗?", function(a) {
									if (a == "yes" && xds.active) {
										xds.inspector
...
var node_change; // 用于 接收 点击的 node
xds.CJsonWindow = Ext.extend(Ext.Window, {
	...
	initComponent : function() {
		...
		function objToCls(obj, dt, lpad) {
			...
			if (s.length == 1)
				obj.name = 'Ext.' + obj.name;// Ext.component
		...
		if (typeof(obj.tbar) != 'undefined') {
			out += lpad + '\tthis.tbar='
					+ objToString(obj.tbar, 'button', lpad + '\t\t', false)
					+ '\n';
		}
		if (typeof(obj.fbar) != 'undefined') {
			out += lpad + '\tthis.fbar='
					+ objToString(obj.fbar, 'button', lpad + '\t\t', false)
					+ '\n';
		}
		if (typeof(obj.bbar) != 'undefined') {
			out += lpad + '\tthis.bbar='
					+ objToString(obj.bbar, 'button', lpad + '\t\t', false)
					+ '\n';
		}
		if (typeof(obj.items) != 'undefined') {
			if (typeof(obj['defaultType']) == 'string') {
				out += lpad
						+ '\tthis.items='
						+ objToString(obj.items, obj['defaultType'], lpad
										+ '\t\t', false) + '\n';
			} else {
				switch (obj['xtype']) {
					case 'viewport' :
						out += lpad
								+ '\tthis.items='
								+ objToString(obj.items, 'panel',
										lpad + '\t\t', false) + '\n';
						break;
					default :
						out += lpad
								+ '\tthis.items='
								+ objToString(obj.items, null, lpad + '\t\t',
										false) + '\n';
						break;
				}
			}
		...
		function objToString(obj, dt, lpad, isW, isClass) {// dt = defaulttype
			...
			if (!isArr && isClass && typeof(obj['name']) == 'string') {// generate
				// class
				// code;
			...
			if (isArr) {
				...
				for (var i = 0; i < len; i++) {
					switch (typeof(obj[i])) {
						...
						case 'object' :
							arr
									.push(objToString(obj[i], dt, lpad + '\t',
											false));
							break;
					}
				}
					...
					items : [new Ext.tree.TreePanel({
								...
								listeners : {
									click : function(n, e) {
										...
										if (!n.parentNode) {
											ed
													.setValue('/*请单击组件树的节点以查看JSON代码.*/');
										} else {
											var js = n.attributes.js;
											node_change = n;
											ed.setValue(objToString(js, null,
													'\t', true, cfg.CLASS));
										}
	...
	onAccept : function() {
		...
		var te = Ext.getCmp('textarea-viewjson-value');
		alert(te.getValue().replace(/(\n)/g, '\r\n'));

		// for (var i = 0; i < this.jdb.length; i++) {
		// rootNode.appendChild(build_node(this.jdb[i]));
		// }
		// node_change.setJs(Ext.decode(te.getValue().replace(/(\n)/g,
		// '\r\n')));
		// this.close()
	},
	getValue : function() {
 ...
 * 
 */
xds.types.TreePanel = Ext.extend(xds.PanelBase, {
			cid : "treepanel",
			defaultName : "&lt;panel&gt;",
			text : "TreePanel(树面板)",
			dtype : "xdtreepanel",
...
Ext.reg("xdtreepanel", xds.TreePanel);
...
xds.types.ButtonGroup = Ext.extend(xds.PanelBase, {
			...
			isValidParent : function(a) {
				return !a || a.getExtComponent().isXType("toolbar")
			}
		});
...
xds.types.GridPanel = Ext.extend(xds.PanelBase, {
			...
			getColumnNodes : function() {
				var f = this.getNode(), c = f.childNodes, d = [];
			...
			doAutoColumns : function() {
				var k = this.getStoreNode(), b = this.getNode();
				if (!k) {
					Ext.Msg.alert("警告", "不能读取列(字段信息)- 数据源(Store)没有定义.");
					return
				}
				xds.canvas.beginUpdate();
				...
				for (var d = 0, f; f = e[d]; d++) {
					var j = f.component, g = j.getConfigValue("type"), a = j
							.getConfigValue("name");
					switch (g) {
						case "int" :
						case "float" :
 ...
 * 自动以组建
 */
xds.types.HTGridPanel = Ext.extend(xds.PanelBase, {
			cid : "htgrid",
			category : "Grid",
			defaultName : "&lt;grid&gt;",
			text : "HT Grid Panel(HT表格面板)",
			dtype : "xdhtgrid",
			xtype : "htgrid",
			xcls : "HT.GridPanel",
			iconCls : "icon-grid",
...
xds.Registry.register(xds.types.HTGridPanel);
xds.HTGridPanel = Ext.extend(HT.GridPanel, {
			
			afterRender : function() {
				xds.HTGridPanel.superclass.afterRender.call(this);
			}
		});
Ext.reg("xdhtgrid", xds.HTGridPanel);
	...
	createStore : function(a, c) {
		...
		if (c) {
			...
			b.on("load", function() {
						...
						xds.status.el.update("");
						this.setSuffix((b.dataCache ? b.dataCache.length : 0)
	...
	createStore : function(a, c) {
		...
		if (c) {
			...
			b.on("load", function() {
						...
						xds.status.el.update("");
						this.setSuffix((b.data.length) + " records loaded",
								"loaded")
					}, this);
			b.proxy.on("loadexception", this.onLoadException, this)
		}
...
xds.types.ComboBox = Ext.extend(xds.FieldBase, {
			...
			layoutable : false,
			enableFlyout : false,
			validChildTypes : ["jsonstore", "arraystore", "xmlstore"],
			isValidChild : function(a) {
				return this.supr().isValidChild.apply(this, arguments)
			},
			defaultConfig : {
...
xds.ComboBox = Ext.extend(Ext.form.ComboBox, {
			getFilmEl : xds.Component.getFilmEl,
			afterRender : function() {
				xds.ComboBox.superclass.afterRender.call(this);
				if (false && this.store && this.store.viewerNode) {
					this.createFloater(this.store.viewerNode.id,
							this.store.storeId, this.store.iconCls)
				}
...
Ext.reg("xcombo", xds.ComboBox);
// / end combobox

// / 定义自己的combo 控件
xds.types.mtdiccombo = Ext.extend(xds.FieldBase, {
			cid : "mtdiccombo",
			defaultName : "&lt;mtdiccombo&gt;",
			text : "ComboBox(数据字典下拉框)",
			dtype : "xmtdiccombo",
			xtype : "mtdiccombo",
			xcls : "MT.DicComboBox",
			iconCls : "icon-combobox",
...
xds.Registry.register(xds.types.mtdiccombo);
xds.mtdiccombo = Ext.extend(MT.DicComboBox, {
			getFilmEl : xds.Component.getFilmEl,
			afterRender : function() {
				xds.mtdiccombo.superclass.afterRender.call(this);
				if (false && this.store && this.store.viewerNode) {
...
Ext.reg("xmtdiccombo", xds.mtdiccombo);
// / end width
