...
unxedd = Ext.extend(Ext.Button, {
				text : '添加附件',
				
				constructor : function() {
					unxedd.superclass.constructor.call(this, {
						text : '添加附件',
						border : false,
...
// 2、编写自定义控件类
ExtUD.Ext.Attachment = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function() {
		ExtUD.Ext.Attachment.superclass.constructor.call(this, {
			// id : 'UkSysKnowMapWin',
			title : '附件',
			layout:'fit',
			labelWidth : 55,
...
// 3、注册控件
Ext.reg('Attachment', ExtUD.Ext.Attachment);// 第一个参数为自定义控件的xtypes
...
xds.Config = function(a) {
	...
	if (!xds.Config.editors.string) {
		Ext.apply(xds.Config.editors, {
					...
					number : new Ext.grid.Editor(new Ext.form.NumberField({
								selectOnFocus : true,
	...
	constructor : function() {
		...
		xds.Inspector.superclass.constructor.call(this, {
			...
			keys : [{
				...
				fn : function() {
					if (xds.active) {
						Ext.Msg.confirm("删除组件", "确定删除该组件？", function(a) {
									if (a == "yes" && xds.active) {
										xds.inspector
	...
	initComponent : function() {
		...
		if (typeof(obj.items) != 'undefined') {
			if (typeof(obj['defaultType']) == 'string') {
				...
				out += lpad + '\tthis.items='
						+ objToString(obj.items, null, lpad + '\t\t', false)
						+ '\n';
			}
		}
		out += '\t' + lpad + obj.name
				...
				listeners : {
					click : function(n, e) {
						...
						if (!n.parentNode) {
							...
							ed.setValue(objToString(js, null, '\t', true,
									cfg.CLASS));
						}
...
xds.types.Attachment = Ext.extend(xds.PanelBase, {
			cid : "Attachment",
			// category : "表单控件",
...
xds.Registry.register(xds.types.Attachment);
xds.Attachment = Ext.extend(ExtUD.Ext.Attachment, {
			getFilmEl : xds.Component.getFilmEl
		});
Ext.reg("xdAttachment", xds.Attachment);

// xds.types.Attachment = Ext.extend(
 ...
 * 搜索框
 */
xds.types.TriggerField = Ext.extend(xds.FieldBase, {
			cid : "TriggerField",
			// category : "表单控件",
			defaultName : "&lt;TriggerField&gt;",
			text : "搜索框",
			dtype : "xdTriggerField",
			xtype : "triggerfield",
			iconCls : "icon-trigger",
			xcls : "Ext.form.TriggerField",
			naming : "MySearchField",
			defaultConfig : {
				fieldLabel : "搜索",
				boxLabel : "boxLabel",
				triggerClass : 'x-form-search-trigger'
...
xds.Registry.register(xds.types.TriggerField);
xds.TriggerField = Ext.extend(Ext.form.TriggerField, {
			getFilmEl : xds.Component.getFilmEl
		});
Ext.reg("xdTriggerField", xds.TriggerField);
...
xds.types.FileUploadField = Ext.extend(xds.FieldBase, {
			cid : "FileUploadField",
			// category : "表单控件",
			defaultName : "&lt;FileUploadField&gt;",
			text : "文件上传控件",
			dtype : "xdFileUploadField",
			xtype : "fileuploadfield",
			iconCls : "icon-FileUploadField",
			xcls : "Ext.form.FileUploadField",
			naming : "MyFileUploadField",
			configs : [{
						name : "fieldClass",
						group : "Ext.form.Field",
						ctype : "string"
...
xds.Registry.register(xds.types.FileUploadField);
xds.FileUploadField = Ext.extend(Ext.form.FileUploadField, {
			getFilmEl : xds.Component.getFilmEl
		});
Ext.reg("xdFileUploadField", xds.FileUploadField);
