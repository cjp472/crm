 ...
 * @class ScBoStockClassifyView
 * @extends Ext.Panel
 * @description [ScBoStockClassify]管理
 * @company 优创融联科技
 * @createtime:
 */
ScBoStockClassifyView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				ScBoStockClassifyView.superclass.constructor.call(this, {
							id : 'ScBoStockClassifyViewWin',
							title : '[ScBoStockClassify]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				var ScBoStockClassifyAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[ScBoStockClassify]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'ScBoStockClassifySearchPanel',
							height : 35,
													items:[
																						 																																												 										{
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new ScBoStockClassifyAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[ScBoStockClassify]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[ScBoStockClassify]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'ScBoStockClassifyGrid',
					url : __ctxPath + "/stock/listScBoStockClassify.do",
					fields : [{
									name : 'boDetailClassifyId',
									type : 'int'
								}
																																																																			,'scBoStockClassify'
																																																																								,'ext1'
					...
					columns:[
								{
									header : 'boDetailClassifyId',
									dataIndex : 'boDetailClassifyId',
									hidden : true
								}
																																																								,{
																	header : '业务单明细内码',
...
//				var searchPanel = Ext.getCmp('ScBoStockClassifySearchPanel');
...
//				}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new ScBoStockClassifyForm({boDetailClassifyId:rec.data.boDetailClassifyId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new ScBoStockClassifyForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ScBoStockClassifyForm');
				if (aForm != null) {
					tabs.remove('ScBoStockClassifyForm');
				}
				aForm = new ScBoStockClassifyForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			removeRs : function(id) {
				$postSubmit({
					url:__ctxPath+ '/stock/multiDelScBoStockClassify.do',
					ids:id,
					grid:this.gridPanel,
					msgTip : '您确认要删除所选记录吗？',
					msgSuccess : '成功删除该记录！',
			...
			editRs : function(record) {
				//new ScBoStockClassifyForm({
				//	boDetailClassifyId : record.data.boDetailClassifyId
				//}).show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ScBoStockClassifyForm');
				if (aForm != null) {
				...
				aForm = new ScBoStockClassifyForm({boDetailClassifyId : record.data.boDetailClassifyId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
			onRowAction : function(grid, record, action, row, col) {
				switch (action) {
					case 'btn-del' :
						this.removeRs.call(this,record.data.boDetailClassifyId);
						break;
					case 'btn-edit' :
