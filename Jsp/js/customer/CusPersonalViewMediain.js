 ...
 * @class CusPersonalViewMediain
 * @extends Ext.Panel
 * @description [CusPersonal]管理
 * @company 优创融联科技
 * @createtime:
 */
CusPersonalViewMediain = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				CusPersonalViewMediain.superclass.constructor.call(this, {
							id : 'CusPersonalViewMediainWin',
							title : '个人客户管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				var fieldnameComboData = [
						...
						['cusTraId', '客户行业：金融、地产、制造等',
								new Ext.form.NumberField({
											name : 'cusTraId',
											allowBlank : true
										})],
						['jobTypId', '客户职位：CEO、CIO、工程师',
								new Ext.form.NumberField({
											name : 'jobTypId',
											allowBlank : true
										})],
						['cusGraId', '客户等级：黑卡、金卡、普卡',
								new Ext.form.NumberField({
											name : 'cusGraId',
											allowBlank : true
										})],
						['hasMarried', '是否已婚', new Ext.form.NumberField({
											name : 'hasMarried',
											allowBlank : true
										})],
						['haveChild', '是否有子女', new Ext.form.NumberField({
											name : 'haveChild',
											allowBlank : true
										})],
...
//								dataIndex : 'ext20'
			...
			// 创建记录
			createRs : function() {
				// new CusPersonalFormMediain().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('CusPersonalFormMediain');
				if (aForm != null) {
					tabs.remove('CusPersonalFormMediain');
				}
				aForm = new CusPersonalFormMediain();
				tabs.add(aForm);
				tabs.activate(aForm);
