...
MyHaveOverServiceView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		MyHaveOverServiceView.superclass.constructor.call(this, {
			id : 'MyHaveOverServiceViewWin',
			title : '我的已结案请求',
			region : 'center',
			layout : 'border',
...
//						url : __ctxPath + '/customer/listlinkmanid.do',
		...
		this.searchPanel = new Ext.FormPanel( {
			layout : 'hbox',
			region : 'north',
			id : 'MyHaveOverConServiceRequestSearchPanel',
			height : 35,
			items :[{
				border:false,
		...
		this.gridPanel = new HT.GridPanel( {
			...
			printable : false,
			exportable : false,
			id : 'MyHaveOverConServiceRequestGrid',
			url : __ctxPath + "/customer/listConServiceRequest.do?Q_status_L_EQ=2&person=" + curUserInfo.userId,
			fields : [ {
				name : 'serviceRequestId',
				type : 'int'
			}, 'customerid', 'type', 'source', 'linkType',
					'busType', 'callNo','customer','customerId', 'accept', 'acceptDate',
					'endtime', 'substatus','status'],
			columns : [ {
	...
	onSearch : function(obj) {
		var searchPanel = Ext.getCmp('MyHaveOverConServiceRequestSearchPanel');
		var gridPanel = Ext.getCmp('MyHaveOverConServiceRequestGrid');
