									...
									'select' : function(combo, record, index) {
										...
										Ext.Ajax.request({
											...
											success : function(response) {
												...
												Ext.getCmp('city').getStore().loadData(result);
...
CustomerForm.prototype.regionList = ['Sheng','Shi'];
...
function getStore_region(flag, idText, parentList){
	...
	return new Ext.data.SimpleStore({
		autoLoad : true,
		url : __ctxPath + '/system/listProvinceRegion.do?regionId='+parentList[flag],
		fields : ['regionId', 'regionName'],
		listeners : {
			load : function() { // 加载数据
				Ext.getCmp(idText+CustomerForm.regionList[flag]+'_combo').setValue(
						Ext.getCmp(idText+CustomerForm.regionList[flag]).getValue());	
			}
		}
	})
}
...
function getListeners_region(flag, idText){
	return {
		select : function(combo, record, index) {
			if(flag < CustomerForm.regionList.length - 1){
				var next = Ext.getCmp(idText + 
						CustomerForm.regionList[flag+1] + '_combo')
				next.clearValue();
				var nextStore = next.getStore();
				Ext.Ajax.request({
					url : __ctxPath + '/system/listProvinceRegion.do',
					params : {
			...
			// 给隐藏域赋值
			Ext.getCmp(idText+CustomerForm.regionList[flag]).setValue(record.get('regionId')); 
		}
	}
}