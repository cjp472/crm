...
MyWantServiceView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		MyWantServiceView.superclass.constructor.call(this, {
			id : 'MyWantServiceViewWin',
			title : '我的待跟踪请求',
			region : 'center',
			layout : 'border',
...
//						url : __ctxPath + '/customer/listlinkmanid.do',
		...
		this.searchPanel = new Ext.FormPanel( {
			layout : 'hbox',
			region : 'north',
			id : 'MyWantConServiceRequestSearchPanel',
			height : 35,
			items : [{
				border:false,
		...
		this.gridPanel = new HT.GridPanel( {
			...
			printable : false,
			exportable : false,
			id : 'MyWantConServiceRequestGrid',
			url : __ctxPath + "/customer/listConServiceRequest.do?Q_status_L_EQ=1&person=" + curUserInfo.userId,
			fields : [ {
				name : 'serviceRequestId',
				type : 'int'
			}, 'customerid', 'type', 'source', 'linkType',
					'busType', 'customer',  'customerId', 'accept', 'acceptDate',
					'starttime', 'note', 'status','callNo'],
			columns : [ {
...
//				}
	...
	onSearch : function(obj) {
		var searchPanel = Ext.getCmp('MyWantConServiceRequestSearchPanel');
		var gridPanel = Ext.getCmp('MyWantConServiceRequestGrid');
