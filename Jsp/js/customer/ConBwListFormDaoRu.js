var ConBwListFormDaoRu = function(){
	var storeLine = null;
	var columnCount = 0;
	var idCount = 0;
	var intCount = 0;
	var getPanel = function(id){
		...
		items:[{
				...
				handler:function(){
					...
					Ext.getCmp('daorutiaojian').remove('bwListPanel'+ii);
	...
	var store = new Ext.data.Store({
				...
				listeners : {
					'load' : function() {
						...
						storeLine = ConBwListFormDaoRu.initFileColumn(columnCount);
...
//							xtype : 'textfield'
		...
		var cm = new Ext.grid.ColumnModel({
			columns : [ {
						...
						editor : new Ext.form.TextField({
							id : 'ConBwList.columnId',
							value : '',
							anchor : '100%'
						}),
						renderer : function() {
							var displayText = '';
							var value = Ext.getCmp('ConBwList.columnId');
		...
		gridPanel = new Ext.grid.EditorGridPanel({
					id : 'ConBwListFormDaoRuGridPanel',
					store : store,
					shim : true,
					height:380,
					autocroll:true,
				...
				items : [{
					...
					items : [{
								...
								items : [{
											...
											xtype : 'mtdiccombo',
											id : 'ConBwListDaoRu.importTypeCombo',
											columnWidth : .8,
											editable : true,
											lazyInit : false,
											allowBlank : false,
											...
											listeners : {
												'select' : function() {
													var combo = Ext.getCmp('ConBwListDaoRu.importTypeCombo');
													var paramObj = {
...
//						}]
					...
					items:[new Ext.FormPanel({
						...
						items : [{
									fieldLabel : '文件类型',
									id : 'ConBwListFormDaoRu.fileType',
									xtype : 'mtdiccombo',
									editable : true,
									lazyInit : false,
									forceSelection : false,
									...
									xtype : 'textfield',
									fieldLabel : '文件路径',
									id : 'ConBwListFormDaoRu.filePath',
									anchor : '80%',
									name : 'filePath',
									inputType : 'file',
									fileUpload : true
...
//	Ext.getCmp('ConBwListDaoRu.importTypeCombo').setValue('1');

	win.show();
}

// 初始化文件列
ConBwListFormDaoRu.initFileColumn = function(columnCount) {
	var columnStr = '[{id:0},';
...
// 确定导入
ConBwListFormDaoRu.toImport = function() {
	var filePath = Ext.getCmp('ConBwListFormDaoRu.filePath').getValue();
	var fileType = Ext.getCmp('ConBwListFormDaoRu.fileType').getValue();
	...
	var store = Ext.getCmp('ConBwListFormDaoRuGridPanel').getStore();
...
//			fileArray.push(fileColumn);
		...
		Ext.Msg.confirm('信息确认', '您确定开始导入吗？', function(btn){
			if (btn == 'yes') {
					Ext.getCmp('ConBwListDaoRu.uploadForm').getForm().submit({
						scope : this,
						...
						success : function(fp, action) {
							...
							$postSubForm({
								formPanel : Ext.getCmp('ConBwListDaoRu.uploadForm'),
								scope : this,
								...
								msgSuccess : '成功导入！',
								msgFailure : '请确认您选择的黑名单模板是否正确,并检查导入值!',
								callback : function(fp, action) {
									if(action!='undefined'&action!=null){
										...
										Ext.getCmp('ConBwListDaoRuhmdWin').close();
									}
