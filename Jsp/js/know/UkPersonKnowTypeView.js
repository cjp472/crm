 ...
 * @class UkPersonKnowTypeView
 * @extends Ext.Panel
 * @description [UkPersonKnowType]管理
 * @company 优创融联科技
 * @createtime:
 */
UkPersonKnowTypeView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				UkPersonKnowTypeView.superclass.constructor.call(this, {
							id : 'UkPersonKnowTypeViewWin',
							title : '[UkPersonKnowType]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				var UkPersonKnowTypeAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[UkPersonKnowType]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'UkPersonKnowTypeSearchPanel',
							height : 35,
													items:[
																						 																																												 										{
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new UkPersonKnowTypeAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[UkPersonKnowType]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[UkPersonKnowType]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'UkPersonKnowTypeGrid',
					url : __ctxPath + "/know/listUkPersonKnowType.do",
					fields : [{
									name : 'personKnowTypeId',
									type : 'int'
								}
																																																																			,'ukPersonKnowType'
																																																																								,'knowTmpId'
					...
					columns:[
								{
									header : 'personKnowTypeId',
									dataIndex : 'personKnowTypeId',
									hidden : true
								}
																																																								,{
																	header : '用户内码',
...
//				var searchPanel = Ext.getCmp('UkPersonKnowTypeSearchPanel');
...
//				}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new UkPersonKnowTypeForm({personKnowTypeId:rec.data.personKnowTypeId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new UkPersonKnowTypeForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('UkPersonKnowTypeForm');
				if (aForm != null) {
					tabs.remove('UkPersonKnowTypeForm');
				}
				aForm = new UkPersonKnowTypeForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			removeRs : function(id) {
				$postSubmit({
					url:__ctxPath+ '/know/multiDelUkPersonKnowType.do',
					ids:id,
					grid:this.gridPanel,
					msgTip : '您确认要删除所选记录吗？',
					msgSuccess : '成功删除该记录！',
			...
			editRs : function(record) {
				//new UkPersonKnowTypeForm({
				//	personKnowTypeId : record.data.personKnowTypeId
				//}).show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('UkPersonKnowTypeForm');
				if (aForm != null) {
				...
				aForm = new UkPersonKnowTypeForm({personKnowTypeId : record.data.personKnowTypeId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
			onRowAction : function(grid, record, action, row, col) {
				switch (action) {
					case 'btn-del' :
						this.removeRs.call(this,record.data.personKnowTypeId);
						break;
					case 'btn-edit' :
