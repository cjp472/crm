 ...
 * 
 */
Ext.ns('ObZuZhiJiGouSelector');

var ObZuZhiJiGouSelector = function() {
	return this.setup();
};

ObZuZhiJiGouSelector.prototype.setup = function(fn) {
	var selected;
...
//	grid.addListener('rowdblclick', rowdblclickFn);
	...
	var treePanel = new Ext.tree.TreePanel({
				...
				rootVisible : false,
				listeners : {
					'click' : ObZuZhiJiGouSelector.clickNode
				}
	...
	function createNode() {
		...
		ObZuZhiJiGouSelector.add(nodeId);
	}
	function deteleNode() {
		var depId = selected.id;
		if (depId > 0) {
			ObZuZhiJiGouSelector.remove(depId);
		} else {
	...
	function editNode() {
		var depId = selected.id;
		if (depId > 0) {
			ObZuZhiJiGouSelector.edit(depId);
		} else {
	...
	var panel = new Ext.Window({
			    ...
			    buttons : [{
						...
						scope : this,
						handler : function(){
			    			ObZuZhiJiGouSelector.submit(fn);
						}
					}, {
						text : '关闭',
						iconCls : 'btn-cancel',
						scope : this,
						handler : ObZuZhiJiGouSelector.close
					}],
				keys : [{
							key : Ext.EventObject.ESC,
							fn : ObZuZhiJiGouSelector.reset,
							scope : this
						}, {
							key : Ext.EventObject.ENTER,
							fn : ObZuZhiJiGouSelector.search,
							scope : this
						}]
			});
	return panel;
 ...
 * Store对象
 */
ObZuZhiJiGouSelector.prototype.initData = function() {
...
ObZuZhiJiGouSelector.add = function(depId) {
		if (depId > 0) {
...
ObZuZhiJiGouSelector.adrList;

ObZuZhiJiGouSelector.edit = function(depId) {
	...
	Ext.Ajax.request({
		...
		success : function(response) {
			...
			ObZuZhiJiGouSelector.adrList = [0, result.data.guojia, result.data.sheng, result.data.shi];
			if (departmentForm == null) {//如果框不存在，则生成一个，然后获取它
				new UlDepartmentForm({
					depid : depId,
					adrList : ObZuZhiJiGouSelector.adrList,
...
ObZuZhiJiGouSelector.multiAction = function(action) {
	...
	if (rows != null && rows.length > 0) {
		...
		ObZuZhiJiGouSelector.action_ids(ids, action);
	} else
		Ext.ux.Toast.msg('操作提示', '对不起，请选择你要操作的数据！');
};

ObZuZhiJiGouSelector.action_ids = function(_ids, action) {
	var actionText = '启用';
 ...
 * 删除组织机构
 */
ObZuZhiJiGouSelector.action = function(depId, action) {
	var actionText = '启用';
	 ...
	 *            回传函数
	 */
ObZuZhiJiGouSelector.submit = function(fn) {
		var jigouNam = '';
		var jigouId = '';
...
//			}
		...
		fn.call(this.scope,jigouNam,jigouId);
		Ext.getCmp('ObZuZhiJiGouSelector').close();
	},

	/**
	 * 关闭当前窗口
	 */
ObZuZhiJiGouSelector.close = function() {
		Ext.getCmp('ObZuZhiJiGouSelector').close();
	}
/**
 * 节点单击事件
 ...
 * @param node
 */
ObZuZhiJiGouSelector.clickNode = function(node) {
	ObZuZhiJiGouSelector.select(node);
};
 ...
 * @param node
 */
ObZuZhiJiGouSelector.select = function(node) {
