/**
 * @description 用户组下拉框多选树
 * @author lijl
 * @updater lijl
 * @createtime 2012年6月19日 12:33:21
 * 
 * @ use 
 *  {
						xtype : 'treecombo',
						...
						anchor : '95%',
						listeners : {
							'collapse' : function(node,check) {//关闭时触发
								alert('实际值:'+Ext.getCmp('tree_tpl').hiddenValue+"[*]显示值:"+Ext.getCmp('tree_tpl').getValue ())
							}
						}
					}
 * 
...
Ext.extend(Ext.tree.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		var buf = [
				...
				'" unselectable="on" />',
				cb
						? ('<input style="height:15px;" class="x-tree-node-cb" type="checkbox" ' + (a.checked
								? 'checked="checked" />'
								: '/>'))
						: '',
...
Ext.ux.TreeCombo = Ext.extend(Ext.form.ComboBox, {
			...
			setHiddenValue : function(code, dispText) {
				this.setValue(code);
				Ext.form.ComboBox.superclass.setRawValue.call(this, code);
			...
			initComponent : function() {
				...
				var fortree = this.tree;
				this.tree.on("check", function(node, checked) {
							// alert(node.text+" = "+checked)
							//var code = node.id;
							_this.setHiddenValue(fortree.getChecked('id'), fortree.getChecked('text'));
							_this.collapse();
						}); // 注册"check"事件
				...
				// // if (node.parentNode &&
				// // node.parentNode.attributes.id != '000000') {
				// var dispText = node.text;
				// var code = node.id;
				// while (node.parentNode
				// && node.parentNode.attributes.id != 0) {
 ...
 * 
 * 使用方法：在loader里加上 baseAttrs:{uiProvider:Ext.tree.TreeCheckNodeUI} 既可. 例如： var
