		...
		var store = new Ext.data.Store({
					...
					reader : new Ext.data.JsonReader({
								...
								fields : [{
											name : 'userId',
											type : 'int'
										}, 'fullname', 'title', 'mobile','ulEmployee']
							}),
					remoteSort : true
				});
...
//							var title = record.data.title;
							if(value != null){
								var title = value.sex;
								if(title == 1)
										return '<img src="'+__ctxPath+'/images/flag/man.png"/>&nbsp;' + value.fullname;
									else if(title == 0)
										return '<img src="'+__ctxPath+'/images/flag/women.png"/>&nbsp;' + value.fullname;	
							}
						},
						width : 60
					}],
					...
					items : [{
								...
								listeners:{
									'select':function(combo,record,index){
										Ext.getCmp('qudaoId').setValue(record.data.itemId);
									}
								},
								maxLength : 256
							},{
		...
		var selectedUserGrid = new Ext.grid.EditorGridPanel({
					...
					store : new Ext.data.ArrayStore({
								autoLoad : true,
								baseParams : {
									userIdStrs : this.userId
								},
								url : __ctxPath
										+ '/system/findbyUserStrAppUser.do',
								fields : [{
											name : 'userId',
											type : 'int'
										}, 'username', 'fullname',
					...
					displayField : 'username,fullname',
					valueField : 'userId',
					trackMouseOver : true,
					sm : csm,
		...
		var PleftPanel = new Ext.Panel({
			...
			items:[{
				xtype:'label',
				html:'<div style="float:right; padding-right:5px;" >请选择用户:</div>',
				layout : 'border',
		...
		var panel = new Ext.Panel({
					...
					border : false,
					anchor : '100%,100%',
					items : [searchPanel, PleftPanel, contactGrid]
...
//			alert(node.id);
	...
	submit : function() {
		...
		if(qudao.getValue() == ''){
			Ext.ux.Toast.msg('操作信息', '请选择一个渠道后发布!');
			return;
		}
		if(userIds == ''){
			Ext.ux.Toast.msg('操作信息', '必须选择用户!');
			return;
		}
		if (this.callback != null)
			this.callback.call(this.scope, userIds, fullnames, qudao.getValue());
