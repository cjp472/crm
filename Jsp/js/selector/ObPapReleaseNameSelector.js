 ...
 * @class ObPapReleaseNameSelector
 * @author 优创融联科技
 * @updater zhangyl
...
ObPapReleaseNameSelector = {
	...
	getView : function(callbackOrConf, isSingle, isForFlow, mobileFlag) {
		...
		var window = new Ext.Window({
					id : 'ObPapReleaseNameSelectorWin',
					title : '选择问卷',
	...
	initPanel : function(isSingle) {
		...
		var store = new Ext.data.Store({
					proxy : new Ext.data.HttpProxy({
								url : __ctxPath + '/pap/papReleasesNamlistPapRelease.do'
							}),
...
//						}
					},
					{
						...
						dataIndex:'papstaid'
//						renderer : function(value) {
//									return CONOB_COM_YWLX.get(value);  状态：有效、注销&PAP_ZT
//								}
					}
					],
					defaults : {
		...
		var searchPanel = new HT.SearchPanel({
					...
					id : 'ObPapReleaseNameSelectorSearchPanel',
					width : 600,
					height : 100,
	 ...
	 */
	search : function() {
		var searchPanel = Ext.getCmp('ObPapReleaseNameSelectorSearchPanel');
...
//		}
	 ...
	 var rows = grid.getSelectionModel().getSelections();
		...
		Ext.getCmp('ObPapReleaseNameSelectorWin').close();
	},
