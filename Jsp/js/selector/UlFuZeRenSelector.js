 ...
 */
  ...
  * {
									...
									text:'选择负责人',
									handler:function(){
										UlFuZeRenSelector.getView(function(userId,fullname){			UlFuZeRenSelector调用UlFuZeRenSelector这个JS
		...
		var store = new Ext.data.Store({
					...
					reader : new Ext.data.JsonReader({
								...
								fields : [{
											name : 'userId',
											type : 'int'
										}, 'fullname','sex','useid','zhiwei','ulDepartment','userNo','deptName']
							}),
					remoteSort : true
				});
...
//					}
		...
		contactGrid.on('rowdblclick', function(grid, rowIndex, e) {
					...
					for (var i = 0; i < rows.length; i++) {
						...
						var deptName=rows[i].data.deptName;
						var isExist = false;
						// 查找是否存在该记录
		...
		var searchPanel = new HT.SearchPanel({
					...
					id : 'UlFuZeRenSelectorSearchPanel',
					width : 600,
					height : 100,
					...
					items : [{
								fieldLabel : '姓名',
								name : 'Q_fullname_S_LK',width : 230,
								flex : 1,
								xtype : 'textfield'
							}, {
								xtype : 'button',
		...
		var selectedEmpGrid = new Ext.grid.EditorGridPanel({
					...
					store : new Ext.data.ArrayStore({
								fields : ['userId', 'fullname','sex','useid','zhiwei','ulDepartment','userNo','deptName']
							}),
					trackMouseOver : true,
					sm : csm,
	 ...
	 */
	search : function() {
		var searchPanel = Ext.getCmp('UlFuZeRenSelectorSearchPanel');
...
//		alert(rows[i].get('useid')+rows[i].get('fullname'));
//		alert(rows[i].get('deptName')+rows[i].get('userNo'));
								}
//	    alert(rows);
		...
		Ext.getCmp('UlFuZeRenSelectorWin').close();
	},
