/**
 * @description 员工选择器(单选)
 * @class UlEmployeeSelector
 * @author 优创融联科技
 * @updater zhangyl
 ...
 */
  ...
  * {
									...
									text:'选择员工',
									handler:function(){
										UlEmployeeSelector.getView(function(userId,fullname){			UlEmployeeSelector调用UlEmployeeSelector这个JS
...
//						}
		...
		var contactGrid = new HT.GridPanel({
					...
					url : __ctxPath + '/xitong/selectsUlEmployee.do',
						fields : [{name : 'userId',
											type : 'int'
										}, 'fullname','sex','useid','zhiwei','ulDepartment','userNo','bankTypeId','bankname'],
					cm : cm
...
//				});
		...
		var selectedEmpGrid = new Ext.grid.EditorGridPanel({
					...
					store : new Ext.data.ArrayStore({
								fields : ['userId', 'fullname','sex','useid','zhiwei','ulDepartment','userNo','bankTypeId','bankname']
							}),
					trackMouseOver : true,
					sm : csm,
	...
	submit : function() {
		...
		if (this.isSingle == null || this.isSingle) {// 选择单个员工
			...
			for (var i = 0; i < rows.length; i++) {
				...
				username+=rows[i].data.userNo;
				bankTypeId +=  rows[i].data.bankTypeId;
				bankname +=  rows[i].data.bankname;
				if (this.mobileFlag) {
		...
		if (this.callback != null)
			this.callback.call(this.scope, userIds, fullnames,sex, useid, zhiwei,depName,depId,userNo,bankTypeId,bankname,username);
		Ext.getCmp('UlEmployeeSelectorWin').close();
	},
