	...
	initPanel : function(isSingle) {
		...
		var store = new Ext.data.Store({
					proxy : new Ext.data.HttpProxy({
								url : __ctxPath + '/outb/callNamlistObCalllist.do?comid='+isSingle
							}),
					...
					fields : [{
								name : ' calllistId',
								type : 'Long'
							}, 'calllistNam','cusTypId','ownerTeam','calllistId','ownerTeamName','calllistResouce','calllistTypId']
							}),
					remoteSort : true
				});
...
//						}
		...
		var searchPanel = new HT.SearchPanel({
					...
					id : 'ObCallNameSelectorSearchPanel',
					width : 600,
					height : 100,
					...
					items : [{
								fieldLabel : '名单名称',
								name : 'Q_calllistNam_S_LK',width : 230,
								flex : 1,
								xtype : 'textfield'
							}, {
								xtype : 'button',
	 ...
	 */
	search : function() {
		var searchPanel = Ext.getCmp('ObCallNameSelectorSearchPanel');
...
//					calllistId += ',';
//				}
//				calllistNam += rows[i].data.calllistNam;
//				cusTypId += rows[i].data.cusTypId;
//				ownerTeam += rows[i].data.ownerTeam;
//				calllistId += rows[i].data.calllistId;
//			}
...
//		}
	 ...
	 var rows = grid.getSelectionModel().getSelections();
		...
		Ext.getCmp('ObCallNameSelectorWin').close();
	},
