...
AssetZhiBiaoView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		AssetZhiBiaoView.superclass.constructor.call(this, {
					id : 'AssetZhiBiaoViewWin',
					title : '监控指标管理',
					region : 'center',
					layout : 'border',
	...
	initUIComponents : function() {
		...
		this.treepanel = new Ext.tree.TreePanel({
			...
			tbar : new Ext.Toolbar({
				items : [{
							...
							text : '刷新',
							handler : function() {
								var tree = Ext.getCmp('Unim_AssTarget_Tree_Id');
			...
			loader : new Ext.tree.TreeLoader({
						url : __ctxPath + '/unim/listTreeUnimAssets.do'
					}),
		...
		this.gridPanel = new HT.GridPanel({
			...
			columns : [{
						...
						header : '资产',
						isExp : false,
						dataIndex : 'unimAssets.assetsName'
					}, {
						header : '状态',
						isExp : false,
						dataIndex : 'status',
...
//				}));
	...
	clickNode:function(node) {
		var users = Ext.getCmp('AssetZhiBiaoViewGrid');
		var store = users.getStore();
		store.url = __ctxPath + "/unim/listUnimAssetsTarget.do";
		store.baseParams = { 
					'Q_unimAssets.assetsId_L_EQ' : node.id>0?node.id:''
				};
		store.reload({
			params : {
				start : 0,
