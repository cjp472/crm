...
BusiCanShuView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		BusiCanShuView.superclass.constructor.call(this, {
					id : 'BusiCanShuViewWin',
					title : '指标参数管理',
					region : 'center',
					layout : 'border',
		...
		this.treepanel = new Ext.tree.TreePanel({
			...
			tbar : new Ext.Toolbar({
				items : [{
							...
							text : '刷新',
							handler : function() {
								var tree = Ext.getCmp('Unim_ChTarPar_Tree_Id');
			...
			loader : new Ext.tree.TreeLoader({
						url : __ctxPath + '/unim/listTreeUnimChaTarPar.do'
					}),
	...
	clickNode:function(node) {
		var users = Ext.getCmp('BusiCanShuViewGrid');
		var store = users.getStore();
		store.url = __ctxPath + "/unim/listUnimChaTarPar.do";
		var nodeId = node.id;
		store.baseParams = { 
					'queryParam' : nodeId
				};
	...
	// 创建记录
	createRs : function() {
		new BusiCanShuForm().show();
//		var tabs = Ext.getCmp('centerTabPanel');
//		var aForm = Ext.getCmp('BusiCanShuForm');
//		if (aForm != null) {
//			tabs.remove('BusiCanShuForm');
//		}
//		aForm = new BusiCanShuForm();
//		tabs.add(aForm);
//		tabs.activate(aForm);
	...
	enableSta : function(id) {
		$postSubmit({
					url : __ctxPath + '/unim/multiEnableUnimChaTarPar.do',
					ids : id,
					grid : this.gridPanel,
					msgTip : '您确认要启用所选记录吗？',
					msgSuccess : '成功启用该记录！',
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			...
			case 'btn-edit' :
				...
				break;
			case 'btn-setting':
				this.enableSta.call(this, record.data.paraId);
				break;
			default :
				break;
		}
