...
UnimAgentMapView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		UnimAgentMapView.superclass.constructor.call(this, {
					id : 'UnimAgentMapViewWin',
					title : '监控视图管理',
					region : 'center',
					layout : 'border',
		...
		this.treepanel = new Ext.tree.TreePanel({
			...
			tbar : new Ext.Toolbar({
				items : [{
							...
							handler : function() {
								var tree = Ext
										.getCmp('UkKnowSiYouViewSearchPanel');
								tree.expandAll();
							}
						}, {
							xtype : 'button',
...
//		Ext.getCmp('UkKnowSiYouViewSearchPanel').on('contextmenu', this.contextmenu, this);
//		this.treePanel.on('contextmenu', this.contextmenu, this);
		...
		this.gridPanel = new HT.GridPanel({
			...
			exportable : false,
			id : 'UnimAgentMapGrid',
			url : __ctxPath + "/unim/listNavingaUnimMapNavigation.do",
			fields : [{
						name : 'mapNavId',
						type : 'Long'
...
//		Ext.getCmp('UnimAgentMapGrid').on('contextmenu', this.contextmenu, this);
 ...
 */
   select : function(node) {
	var users = Ext.getCmp('UnimAgentMapGrid');
	var store = users.getStore();
	store.url = __ctxPath + '/unim/listNavingaUnimMapNavigation.do';
	store.baseParams = { 
...
//				'Q_phoneGroup.isPublic_SN_EQ':0
	...
	createNode:function(){
	    ...
	    new UnimAgentMapForm({parentid:parentid}).show();
	},
	editNode:function(){
	    var parentid = this.selectedNode.id;
		if (parentid > 0) {
			new UnimAgentMapForm({mapNavId:parentid}).show();
		} else {
			Ext.MessageBox.show({
				title : '操作信息',
				msg : '该处不能被修改',
	...
	deleteNode:function(){
	    ...
	    var treePanel=Ext.getCmp('UkKnowSiYouViewSearchPanel');
		Ext.Ajax.request({
							...
							if (btn == 'yes') {
								Ext.Ajax.request({
									...
									success : function(result, request) {
	                                    ...
	                                    var store= Ext.getCmp('UnimAgentMapGrid').getStore();
										store.reload();
									},
	...
	// 创建记录
	createRs : function() {
		var node = Ext.getCmp('UkKnowSiYouViewSearchPanel').getSelectionModel().getSelectedNode();
		if(node!=null){
	...
	removeRs : function(id) {
		var rows = Ext.getCmp("UnimAgentMapGrid").getSelectionModel().getSelections();
		var huosta = rows[0].data.status;
		if (huosta == 1) {
			$gridRs( {
				url : __ctxPath + '/unim/zhuXiaoUnimMapNavigation.do',
				grid : this.gridPanel,
//				ids : id,
				idName : 'mapNavId',
				msgNull : '请选择要注销的记录！',
...
//					ids : id,
	...
	qiyonglRs : function(id) {
		var rows = Ext.getCmp("UnimAgentMapGrid").getSelectionModel().getSelections();
				    $gridRs( {
					url : __ctxPath + '/unim/qiYongUnimMapNavigation.do',
					grid : this.gridPanel,
//					ids : id,
					idName : 'mapNavId',
					msgNull : '请选择要启用的记录！',
