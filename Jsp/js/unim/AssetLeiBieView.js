...
AssetLeiBieView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		AssetLeiBieView.superclass.constructor.call(this, {
					id : 'AssetLeiBieViewWin',
					title : '资产类别管理',
					region : 'center',
					layout : 'border',
...
//							},
		...
		this.gridPanel = new HT.GridPanel({
			...
			fields : [{
						name : 'typeId',
						type : 'Long'
					}, 'typeId', 'typeName', 'typeCode', 'extend1',
					'status'],
			columns : [{
						header : 'typeId',
						dataIndex : 'typeId',
 ...
 */
   select : function(node) {
	var users = Ext.getCmp('UnimAssTypeGrid');
	var store = users.getStore();
	store.url = __ctxPath + '/unim/listUnimAssType.do';
	store.baseParams = { 
...
//				'Q_phoneGroup.isPublic_SN_EQ':0
	...
	// 创建记录
	createRs : function() {
		new AssetLeiBieForm().show();
//		var tabs = Ext.getCmp('centerTabPanel');
//		var aForm = Ext.getCmp('AssetLeiBieForm');
//		if (aForm != null) {
//			tabs.remove('AssetLeiBieForm');
//		}
//		aForm = new AssetLeiBieForm();
//		tabs.add(aForm);
//		tabs.activate(aForm);
	...
	removeRs : function(id) {
			$gridRs( {
				url : __ctxPath + '/unim/zhuXiaoUnimAssType.do',
				grid : this.gridPanel,
//				ids : id,
				idName : 'typeId',
				msgNull : '请选择要注销的记录！',
