...
AssetMapView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		AssetMapView.superclass.constructor.call(this, {
					id : 'AssetMapViewWin',
					title : '监控视图管理',
					region : 'center',
					layout : 'border',
	...
	initUIComponents : function() {
		this.treepanel = new Ext.tree.TreePanel({
			...
			height : 800,
			width : 180,
			id : 'UnimAssetsNavigationSearchPanel',
			tbar : new Ext.Toolbar({
				items : [{
							...
							handler : function() {
								var tree = Ext
										.getCmp('UnimAssetsNavigationSearchPanel');
								tree.expandAll();
							}
						}, {
							xtype : 'button',
...
//		Ext.getCmp('UnimAssetsNavigationSearchPanel').on('contextmenu', this.contextmenu, this);
//		this.treePanel.on('contextmenu', this.contextmenu, this);
		...
		this.topbar = new Ext.Toolbar({
					items : ['->',{
								...
								iconCls : 'btn-del-double',
								// text : __delete+'[UnimAgentMap]',
								text : '批量删除',
								xtype : 'button',
								scope : this,
								handler : this.zhuXiaoRs
							}]
				});
		...
		this.gridPanel = new HT.GridPanel({
			...
			printable : false,
			exportable : false,
			id : 'UnimAssetsNavigationGrid',
			url : __ctxPath + "/unim/listNavingaUnimAssetsNavigation.do",
			fields : [{
						name : 'mapNavId',
						type : 'Long'
...
//		Ext.getCmp('UnimAssetsNavigation').on('contextmenu', this.contextmenu, this);
 ...
 */
   select : function(node) {
	var users = Ext.getCmp('UnimAssetsNavigationGrid');
	var store = users.getStore();
	store.url = __ctxPath + '/unim/listNavingaUnimAssetsNavigation.do';
	store.baseParams = { 
...
//				'Q_phoneGroup.isPublic_SN_EQ':0
	...
	createNode:function(){
	    ...
	    new AssetMapForm({parentid:parentid}).show();
	},
	editNode:function(){
	    var parentid = this.selectedNode.id;
		if (parentid > 0) {
			new AssetMapForm({mapNavId:parentid}).show();
		} else {
			Ext.MessageBox.show({
				title : '操作信息',
				msg : '该处不能被修改',
	...
	deleteNode:function(){
	    ...
	    var treePanel=Ext.getCmp('UnimAssetsNavigationSearchPanel');
		Ext.Ajax.request({
			...
			success : function(result, request) {
				...
				Ext.Msg.confirm('删除操作', '组里还有' + count + '个地图，你确定删除目录组吗?',
						function(btn) {
							if (btn == 'yes') {
								Ext.Ajax.request({
									url : __ctxPath + '/unim/multiDelUnimMapNavigation.do',
									params : {
										ids : groupId
									},
									method : 'post',
									success : function(result, request) {
	                                    ...
	                                    var store= Ext.getCmp('UnimAssetsNavigationGrid').getStore();
										store.reload();
									},
	...
	// 创建记录
	createRs : function() {
		var node = Ext.getCmp('UnimAssetsNavigationSearchPanel').getSelectionModel().getSelectedNode();
		if(node!=null){
		...
		//单条数据操作（注销\删除）
	zhuXiao : function(id) {
		var rows = Ext.getCmp("UnimAssetsNavigationGrid").getSelectionModel().getSelections();
		for (var i = 0; i < rows.length; i++) {
		var huosta = rows[i].data.status;
		if (huosta == 1) {
			$gridRs( {
				url : __ctxPath + '/unim/zhuXiaoUnimAssetsNavigation.do',
				grid : this.gridPanel,
//				ids : id,
				idName : 'mapNavId',
				msgNull : '请选择要注销的记录！',
		...
		if (huosta == 0 || huosta==2) {
				$gridRs( {
					url : __ctxPath + '/unim/multiDelUnimAssetsNavigation.do',
					grid : this.gridPanel,
//					ids : id,
					idName : 'mapNavId',
					msgTip : '您确认要删除所选记录吗？',
...
//		if(huosta == 2){
//			Ext.ux.Toast.msg('操作信息', '该记录已注销！');
//		}
		}

	},
	//批量删除
	zhuXiaoRs : function(id) {
				$gridRs( {
					url : __ctxPath + '/unim/multiDelUnimAssetsNavigation.do',
					grid : this.gridPanel,
//					ids : id,
	...
	qiyonglRs : function(id) {
		var rows = Ext.getCmp("UnimAssetsNavigationGrid").getSelectionModel().getSelections();
				    $gridRs( {
					url : __ctxPath + '/unim/qiYongUnimAssetsNavigation.do',
					grid : this.gridPanel,
//					ids : id,
					idName : 'mapNavId',
					msgNull : '请选择要启用的记录！',
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-del' :
				this.zhuXiao.call(this, record.data.mapNavId);
				break;
			case 'btn-del-double' :
				this.zhuXiaoRs.call(this, record.data.mapNavId);
				break;
