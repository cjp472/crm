TaskNeedsDateWindow=Ext.extend(Ext.Window,{
	constructor:function(conf){
		Ext.applyIf(this,conf);
		TaskNeedsDateWindow.superclass.constructor.call(this,{
			title:'为任务设置限期时间',
			height:120,
			width:350,
			layout:'form',
	...
	save:function(){
		...
		var taskGrid=this.taskGrid?this.taskGrid:Ext.getCmp(this.grid);   //'allTaskGrid'
		var rs = taskGrid.getSelectionModel().getSelections();
		var taskIds='';
		for(var i=0;i<rs.length;i++){
			if(i>0)taskIds+=',';
//			taskIds+=rs[i].data.taskId;
			taskIds+=rs[i].data.runId;
		}
		if(dueDate.getValue()==''){
		...
		//取到选择的记录行数
		Ext.Ajax.request({
			url:__ctxPath+'/flow/updateNeedsTask.do',
			method:'POST',
			scope:this,
			params:{
				taskIds:taskIds,
