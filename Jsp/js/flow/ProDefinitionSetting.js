...
//	 			}
	...
	getFormTemplateGrid:function(){
		...
		this.formTempGridPanel = new HT.EditorGridPanel({
			...
			fields : ['templateId', 'mappingId','nodeName'],
			columns : [{
						header : 'templateId',
 ...
 ProDefinitionSetting.prototype.usersPanel=function(defId){
	...
	var store = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
					root : 'result',
					id : 'id',
					fields : ['assignId', 'deployId', 'activityName','reJobId','reJobName',
							'userId','username', 'roleId','roleName','jobId','jobName','isSigned','requiredTime']
				})
			}); 
	...
	var reJobEditor = new Ext.form.TriggerField({
		...
		onTriggerClick : function(e){
			...
			RelativeJobSelector.getView(function(ids, names){
				record.set('reJobId',ids);
				record.set('reJobName',names);
			},false).show();
			grid.stopEditing();
		}
...
//		triggerClass : 'x-form-browse-trigger',
//		onTriggerClick : function(e){
//			JobSelector.getView(function(ids,names){
//				var store = grid.getStore();
//				var record = store.getAt(row);
//				//设置相对岗位信息
...
//	}); // end of this jobEditor
	...
	var jobEditor = new Ext.form.TriggerField({
		triggerClass : 'x-form-browse-trigger',
		onTriggerClick : function(e){
			UsergroupSelector.getView(function(ids,names){
				var store = grid.getStore();
				var record = store.getAt(row);
				//设置相对岗位信息
				record.set('jobId',ids);
...
//	if(nodeType=='decision'){//弹出设置条件的窗口
//		$ImportSimpleJs([__ctxPath+'/js/flow/ProDecisionSetting.js'],function(){
//			new ProDecisionSetting({
//				defId:defId,
//				activityName:activityName
//			}).show();
//		},this);
