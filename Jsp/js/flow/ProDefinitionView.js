...
//				}, 
				{
					...
					iconCls : 'btn-reset',
					handler : function() {
						var searchPanel = Ext.getCmp('ProDefinitionSearchForm');
...
ProDefinitionView.prototype.setup = function() {
	...
	var cm = new Ext.grid.ColumnModel({
		columns : [sm, new Ext.grid.RowNumberer(), {
					...
					header : '工单类别',
					dataIndex : 'busClasses',
					hidden : true,
					renderer : function(value) {
						return FLOW_BUSCLASSES.get(value);
					}
				}, {
					header : '描述',
					...
					renderer : function(value, metadata, record, rowIndex,
						...
						if (isManager) {
							...
							if (isGranted('_FlowEdit') && drawDefXml!=null) {
								str += '&nbsp;<button title="流程设计" value=" " class="btn-flow-design" onclick=" ProDefinitionView.reDesign('+ defId + ')"></button>';
							}
						...
						if (deployId != null) {
							...
							//str += '&nbsp;<button title="新建流程" class="btn-newFlow" onclick="ProDefinitionView.newFlow('+ '' + defId + ',\'' + name + '\')"></button>';
						}
						if (isManager) {
	...
	var store = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
							...
							fields : [{
									...
									'createtime', 'deployId','status','drawDefXml','busClasses','busType']
						}),
				remoteSort : true
			});
