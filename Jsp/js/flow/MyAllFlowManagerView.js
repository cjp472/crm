Ext.ns('MyAllFlowManagerView');
/**
 * 我的任务流程
 */
var MyAllFlowManagerView = function() {
	var searchPanel = new HT.SearchPanel({
				layout : 'hbox',
				region : 'north',
				id : 'MyAllFlowFlowManagerSearchForm',
				// colNums : 3,
				height : 35,
				items : [{
							...
							handler : function() {
								var searchPanel = Ext
										.getCmp('MyAllFlowFlowManagerSearchForm');
								var gridPanel = Ext.getCmp('MyAllFlowGrid');
...
MyAllFlowManagerView.prototype.setup = function() {
	// 显示一个GridPanel先即可
		...
		columns : [new Ext.grid.RowNumberer(), {
				...
				// },
				{
					...
					renderer : function(value, metadata, record, rowIndex,
						...
						var str = '';
						str += '<button title="查看" class="btn-detail" onclick="MyAllFlowManagerView.trackRs(\''
								+ flowPk
								+ '\',\''
								+ runId
								+ '\',\''
	...
	var grid = new Ext.grid.GridPanel({
				...
				// iconCls : 'btn-refresh',
				// handler : function() {
				// Ext.getCmp('MyAllFlowGrid').getStore().reload();
				// }
				// }]
				// }),
				cm : cm,
	...
	// MyAllFlowManagerView.nextStep(rec.data.taskId,rec.data.activityName);
	// });
	// });
...
MyAllFlowManagerView.prototype.initData = function() {
	var store = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
							...
							fields : ['flowPk', 'runId', 'flowType', 'creater',
									'piId', 'taskUser', 'taskName', 'dueDate',
									'busType', 'busClasses', 'createTime',
									'acceptanceTime', 'taskId', 'runStatus',
									'needsTime', 'finishTime', 'taskUserName',
									'applyName', 'applyTime', 'residueTime',
									'runType']
						}),
				remoteSort : true
			});
	store.setDefaultSort('dbId', 'desc');
	store.setBaseParam('runStatus', 2);
	store.setBaseParam('creater', curUserInfo.userId);
	return store;
};
 ...
 *            taskdbid
 */
MyAllFlowManagerView.lockTask = function(taskId) {
	Ext.Ajax.request({
				...
				success : function(result, response) {
					var grid = Ext.getCmp("MyAllFlowGrid");
 ...
 *            taskId
 */
MyAllFlowManagerView.changeTask = function(taskId, taskname) {
 ...
 *            taskdbid
 */
MyAllFlowManagerView.unlockTask = function(taskId) {
	Ext.Ajax.request({
 ...
 *            taskdbid
 */
MyAllFlowManagerView.nextStep = function(taskId, activityName) {
...
MyAllFlowManagerView.trackRs = function(flowPk, runId, fm) {
