DDHuiFangView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		DDHuiFangView.superclass.constructor.call(this, {
			id : 'DDHuiFangViewWin',
			title : '订单回访管理',
			region : 'center',
			layout : 'border',
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'callme' :
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('DDHuiFangFormWin');
				if (aForm != null) {
					tabs.remove('DDHuiFangFormWin');
				}
				aForm = new DDHuiFangForm({
					id:record.data.calllistId
				});
				tabs.add(aForm);
				tabs.activate(aForm);
				break;
