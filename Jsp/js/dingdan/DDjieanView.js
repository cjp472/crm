 ...
 * @createtime:
 */
DDjieanView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				DDjieanView.superclass.constructor.call(this, {
							id : 'DDjieanViewWin',
							title : '已结案订单',
							region : 'center',
							layout : 'border',
					...
					case 'btn-form-design' :
						Ext.Ajax.request({
                            ...
                            success : function(response) {
                                var tabs = Ext.getCmp('centerTabPanel');
                                var aForm = Ext.getCmp('DDDetailWin');
                                if (aForm != null) {
                                    tabs.remove('DDDetailWin');
                                }
                                aForm = new DDDetail({
                                    data_dd : Ext.util.JSON.decode(response.responseText)
                                });
                                tabs.add(aForm);
                                tabs.activate(aForm);
                            }
                        });
//                        ScBizOrderSalesSelector.getView(
//                            function(){
//                                
//                            },false
//                        ).show();
