...
function newViewf(viewName,params,node){
	var str='new ' + viewName ;
		str+='({id:\'' + node.id+'\','
			   +'title:\''+node.attributes.text+'\','
			   +'params:\''+node.attributes.params+'\','
			   +'flowNode:\''+node.attributes.flowNode+'\'});';
	return eval(str);
}
...
function $ImportJsf(viewName,callback,params,node) {
	var b = jsCache[viewName];
	if (b != null) {
		var view =newViewf(viewName,params,node);
		callback.call(this, view);
	} else {
		var jsArr = eval('App.importJsf.' + viewName);
		if(jsArr==undefined || jsArr.length==0){
			try{
				var view = newViewf(viewName,params,node);
//				alert(view +' line 91');
 ...
 * @return {}
 */
function $converCmp(fElements,jsonData,rightJson,addFlag,formId){
	    var flag='on';
...
//	                }
			...
			if(right&&(right==0||right==1)&&(xtype!='officeeditor'||(xtype=='officeeditor'&&right==0))){
				flag='un';
...
//				formPanel.add(cmp);
			    ...
			    try{
					...
					map.add('officeeditor'+index+'-cmp',cmp.panel);
...
//					      if(!flag){
//					      	 var hiddenF=new Ext.form.Hidden({
//							    name:gridName+'_'+pkKeyVar,
//							    value:pkKeyValue
//							 });
//					         this.formPanel.add(hiddenF);
...
//		       div_1_1.setAttribute('style','background: #F8F8F8 no-repeat scroll left bottom transparent;border:0 none;');
			   ...
			   try{
			   var r=$converCmp.call(this,form2.elements,datas[i],rightJson,true);
			   if(r=='un'){
			      flag=false;
			   }
			   }catch(e){alert(e);}
...
function $converFormDetail(formobjs){
	   for(var i=0;i<formobjs.length;i++){
           $converForm.call(this,formobjs[i]);
	   }
};
...
//					var subTableId=document.getElementById('subPkName_'+this.runId);
				...
				//加载其列表
				if(jsonData){
//					var data=jsonData[detailVar.value];
					var data=jsonData['WF_'+gridName+'s'];
					if(data){
						this.detailPanel.getStore().loadData({result:data});
					}
				}
//				var detailVar=document.getElementById('subSetVarName_'+this.runId);
//				if(jsonData && detailVar){
//					var data=jsonData[detailVar.value];
//					if(data){
//						this.detailPanel.getStore().loadData({result:data});
//					}
//				}
				parent.removeChild(tables[i]);
				break;
			}else if(isdetail!=null&&'false'==isgrid){
			    var names=$getTableInputCmpName(tables[i]);
				if(this.runId){
...
/** 用来遍历指定对象所有的属性名称和值,obj 需要遍历的对象  */ 
AppUtil.showObj = function ( obj ) { 
    // 用来保存所有的属性名称和值 
    var props = "props: \n" ;
    var func = "\n\nfunctions: \n";
    // 开始遍历 
    for ( var p in obj ){ 
        // 方法 
        if ( typeof ( obj [ p ]) ==  'function' ){//使用"function"时,一直进不来
            func += p + ' : ' + obj [ p ] + " \n" ; 
        } else { 
...
/** 用来遍历指定对象所有的属性名称和值,obj 需要遍历的对象  */ 
AppUtil.objToString = function ( obj ) { 
    // 用来保存所有的属性名称和值 
    var str = "" ;
    // 开始遍历 
    for ( var p in obj ){ 
...
AppUtil.synchroAjax = function (url, data) { 
    var responsea = Ext.lib.Ajax.getConnectionObject().conn;
    responsea.open("POST",  __ctxPath + '/system/proxyRestfulWSProxy.do', false);
    responsea.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
    responsea.send(AppUtil.objToString(data));
    var result = Ext.util.JSON.decode(responsea.responseText);
    return result;
};

AppUtil.interfaceBase = { 
    'subsystem' : __interface.sys,
    'account' : __interface.account,
    'password' : __interface.password   
};

AppUtil.copy = function ( obj , target) { 
//    if(target instanceof Array){
//          var st_da = Ext.util.JSON.encode(target);
//          alert(st_da);
//          target = new Object();
//          target['cols'] = st_da;
//          for ( var p in obj ){ 
...
AppUtil.proxyURL = __interface.url;
AppUtil.ulr_showPic = __interface.url_pic;
//AppUtil.proxyURL = 'http://192.168.233.31:80/Interface/service';

Ext.ns('AppUtil.address'); 
AppUtil.address.regionTypeList = ['Sheng', 'Shi', 'Qu', 'Jiedao'];

AppUtil.address.getStore_region = function(flag, idText, parentList) {
	if (flag != 0) {// 除第一项外，其他项的store设为空白
	    return new Ext.data.SimpleStore({
	                fields : ['name', 'data']
			    });
    }
    var response = Ext.lib.Ajax.getConnectionObject().conn;
    response.open("POST",  __ctxPath + '/system/listNewRegion.do?parentId=0&typeId=1', false);
    response.send();
    var result = Ext.util.JSON.decode(response.responseText);
    var data = [];
    for(var i = 0; i < result.count; i++){
        data.push([result.data[i].regionName, {
            id : result.data[i].regionId,
            areaNo : result.data[i].areaNo
        }]);
    }
...
AppUtil.address.getListeners_region = function(flag, idText) {
    return {
        select : function(combo, record, index) {
            if (flag < AppUtil.address.regionTypeList.length - 1) {
                var next = Ext.getCmp(idText + AppUtil.address.regionTypeList[flag + 1]
                        + '_combo')
                next.clearValue();
                var nextStore = next.getStore();
                Ext.Ajax.request({
                        url : __ctxPath + '/system/listNewRegion.do',
                        params : {
                            parentId : record.data.data.id,
                            typeId : flag + 1 
                        },
                        method : 'post',
                        success : function(response) {
                            ...
                            nextStore.loadData(data);
                            next.setValue('请选择');
                        }
                    });
            }else{//最后一级,填充邮政编码
                if(Ext.getCmp('deliveryPost') == undefined)
                    return;
                Ext.Ajax.request({
                    url : __ctxPath + '/system/getDetailRegion.do',
                    params : {
                        regionId : record.data.data.id
                    },
                    method : 'post',
                    success : function(response) {
                        var result = Ext.util.JSON.decode(response.responseText);
                        Ext.getCmp('deliveryPost').setValue(result.data.postCode);
                    }
                });    
            }
        }
...
//只存在三级地址的级联框的方法
Ext.ns('AppUtil.address2');
AppUtil.address2.regionList = ['sheng','shi','qu'];
AppUtil.address2.regionList_GS = ['GSsheng','GSshi','GSqu'];

AppUtil.address2.getStore_region = function (flag, idText, parentList){
	
    if(parentList == null || parentList == undefined){//parentList不存在,说明为新增的时候
        if(flag != 0){//新增时，非第一个下拉框，store都为空白
            return new Ext.data.SimpleStore({
                fields : ['regionId', 'regionName']
            });
        }else{//新增时，第一个下拉框的父节点为0
            parentList = [0];
        }
    }
    return new Ext.data.SimpleStore({
        autoLoad : true,
        url : __ctxPath + '/system/listDetailNewRegion.do?regionId='+parentList[flag] + '&type=' + (flag+1),
        fields : ['regionId', 'regionName'],
        listeners : {
            load : function() { // 数据加载完成后，根据对应的隐藏域去设置下拉框的值
                var comId = idText+AppUtil.address2.regionList[flag]+'_combo';
                var hidId = idText+AppUtil.address2.regionList[flag];
//                if(!Ext.isEmpty(Ext.getCmp(hidId).getValue())){
//                    Ext.getCmp(comId).setValue(Ext.getCmp(hidId).getValue());
//                }   
            }
        }
    })
...
AppUtil.address2.getListeners_region = function (flag, idText , name , type){
    return {
        select : function(combo, record, index) {
            if(flag < AppUtil.address2.regionList.length - 1){
            	var next;
            	if(type == 0){
            		next = Ext.getCmp(idText + AppUtil.address2.regionList[flag+1] + '_combo');
            	}else{
            		 next = Ext.getCmp(idText + AppUtil.address2.regionList_GS[flag+1] + '_combo');
            	}
               
                next.clearValue();
                var tmp_flag = flag;
                tmp_flag++;
                while(tmp_flag < AppUtil.address2.regionList.length - 1){
                    var tmp = Ext.getCmp(idText + AppUtil.address2.regionList[tmp_flag+1] + '_combo');
                    tmp.clearValue(); 
                    tmp_flag++;
                }
                var nextStore = next.getStore();
            ...
            // 给隐藏域赋值
            // Ext.getCmp(idText+AppUtil.address2.regionList[flag]).setValue(record.get('regionId')); 
            
            //alert("省市对应的id：" + record.get('regionId'));
//            省的同步
            if(type == 0){
            	sysAdd(name,record.get('regionId'));
            }else {
            	GS_sysAdd(name , record.get('regionId'));
            }
           // sysAdd(name,record.get('regionId'));
        }
    }
 ...
 * */
var next="";
AppUtil.address2.getRegionId = function (parenId,flag){
	 if(flag == 1){
	 	next =  Ext.getCmp('ulEmployee.hujishi_combo');
	 }else if(flag == 2){
	 	next =  Ext.getCmp('ulEmployee.hujiqu_combo');
	 }
	 var nextStore = next.getStore();
	 Ext.Ajax.request({
                    url : __ctxPath + '/system/listDetailNewRegion.do', 
...
//	 return new Ext.data.SimpleStore({
//        autoLoad : true,
//        url : __ctxPath + '/system/listDetailNewRegion.do?regionId=' + parenId + '&type=' + (flag+1),
//        fields : ['regionId', 'regionName'],
//        listeners : {
//            load : function() { // 数据加载完成后，根据对应的隐藏域去设置下拉框的值
...
/**
 * 
 * @param {} mapName  LSXF001 数据字典英文
 * @param {} itemIndex  数据字典传入后台的值
 * @param {} subId 需要修改的子项item的id
 */
AppUtil.getSubStore = function(mapName, itemIndex, subId, callback, value){
	Ext.Ajax.request({
		url : __ctxPath + '/system/dicIdByComboDictionary.do?Q_mapName_S_EQ='+mapName,
		params : {
		...
		success : function(response) {
			var result = Ext.util.JSON.decode(response.responseText);
			var dicId = result.dicId;
			
			Ext.Ajax.request({
				url : __ctxPath + '/system/comboByDicIdDictionary.do',
				params : {
					'Q_relDic_L_EQ' : dicId
				},
				method : 'post',
				success : function(response) {
					var result = Ext.util.JSON.decode(response.responseText);
					Ext.getCmp(subId).getStore().loadData(result);
					Ext.getCmp(subId).clearValue();
 ...
 * @param {} subId 需要修改的子项item的id
 */
AppUtil.getCascadeStore = function(mapName, itemIndex, subId, subMapName , callback, value){
	Ext.Ajax.request({
		url : __ctxPath + '/system/dicIdByComboDictionary.do?Q_mapName_S_EQ='+mapName,
		params : {
			'Q_itemIndex_S_EQ' : itemIndex
