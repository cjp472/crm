...
function $getGdSelectedIdsDel(grid, idName,delIds) {
	var selRs = grid.getSelectionModel().getSelections();
	var ids = Array();
	var delIdsArr = delIds.split(',');
	var flag = false;
	for (var i = 0; i < selRs.length; i++) {
		for(var j=0;j<delIdsArr.length;j++){
			if(delIdsArr[j] == eval('selRs[i].data.' + idName)){
				flag = true;
			}
		}
	if(flag) ids.push(eval('selRs[i].data.' + idName));
	}
	return ids;
}
function $postFabu(conf) {
	var record = conf.grid.getSelectionModel().getSelected();
	var status = record.data.status;
	if (status == 1) {
		Ext.ux.Toast.msg('操作信息', '该话题已发布！');
	} else {
		Ext.Msg.confirm('信息确认', '您确认要发布所选记录吗？', function(btn) {
					if (btn == 'yes') {
						Ext.Ajax.request({
									url : conf.url,
									params : {
...
function $delGridRsFubu(conf) {
	...
	if (record == null) {
		...
		var status = record.data.status;
		if (status == 1) {
			Ext.ux.Toast.msg('操作信息', '已发布话题，不能进行删除！');
		} else {
			var status = $getGdSelectedStatus(conf.grid, conf.idName);
			if (status == 1) {
				Ext.ux.Toast.msg('操作信息', '包含已发布话题，不能进行删除！')
			} else {
...
function $getGdSelectedStatus(grid, idName) {
	var selRs = grid.getSelectionModel().getSelections();
	var status;
	if (selRs.length > 1) {
		for (var i = 0; i < selRs.length; i++) {
			if (selRs[i].data.status == 1) {
				status = 1;
			}
			// status.push(eval('selRs[i].data.' + idName));
		}
	} else {
		status = 2;
	}
...
function $postDelFabu(conf) {
	var record = conf.grid.getSelectionModel().getSelected();
	var status = record.data.status;
	if (status == 1) {
		Ext.ux.Toast.msg('操作信息', '该话题已发布不能进行删除！');
	} else {
	...
	if (conf.formPanel.getForm().isValid()) {
		...
		conf.formPanel.getForm().submit({
					...
					success : function(form, action) {
						...
						if (conf.callback) {
							conf.callback.call(scope, form, action);
						}
					},
...
function $postForms(conf, conf1) {
	if (conf.formPanel.getForm().isValid()) {
		...
		conf.formPanel.getForm().submit({
					...
					url : conf.url,
					method : 'post',
					params : [conf.params, conf1.params],
					waitMsg : '正在提交数据...',
					success : function(fp, action) {
						Ext.ux.Toast.msg('操作信息', '成功保存信息！');
						if (conf.callback) {
...
function $delGridRs(conf) {
	var ids = [];
	if(conf.delFlag){
		ids = $getGdSelectedIdsDel(conf.grid, conf.idName,conf.delIds);
		
	}else{
		ids = $getGdSelectedIds(conf.grid, conf.idName);
...
function $search(conf) {
	...
	if (searchPanel.getForm().isValid()) {// 如果合法
		...
		deParams.start = 0;
		for(var item in store.baseParams){
			if(item != 'start' && !(item in deParams)){
				deParams[item] = store.baseParams[item];
			}
		}
 ...
 * 显示预览按钮
 */
HT.ux.plugins.showPreview = Ext.extend(Object, {
			constructor : function(config) {
				Ext.apply(this, config);
				HT.ux.plugins.showPreview.superclass.constructor.call(this,
						config);
			},
...
function CommonExport(gridObj, isExportAll, exportType) {
	...
	var cols = gridObj.getColumnModel().config.columns ? gridObj
			.getColumnModel().config.columns : gridObj.getColumnModel().config;
	...
	for (var index = 0; index < cols.length; index++) {
		if (index > 2 && cols[index].isExp == true) {
			...
			if (cols[index].javaRenderer != null) {
				...
				// alert("caoli:"+colId);
			} else if (cols[index].mapRenderer != null) {
				// alert(cols[index].mapRenderer+'caoli'+cols[index].value+''+ORDERSOURCEMAP.get(1));
				// colId +="mapRenderer:"+cols[index].mapRenderer+",";
				colId += cols[index].dataIndex + ',';
				mapRenderer += cols[index].dataIndex + "@"
						+ cols[index].mapRenderer + ",";
				// alert("CAOLI:"+colId);
				// alert("CAOLI:MAP"+mapRenderer);
				// colId+= cols[index].mapRenderer.get(cols[index].value);
			} else {
				colId += cols[index].dataIndex + ',';
				// alert(colId);
			}
	...
	if (colName.length > 0) {
		...
		mapRenderer = mapRenderer.substring(0, mapRenderer.length - 1)
		// alert("colId:"+colId+",colName:"+colName+",mapRenderer:"+mapRenderer);
	}
 ...
 * @extends Object
 */
HT.ux.plugins.Email = Ext.extend(Object, {
	constructor : function(config) {
		Ext.apply(this, config);
		HT.ux.plugins.Email.superclass.constructor.call(this, config);
	},
 ...
 * @extends Object
 */
HT.ux.plugins.Fox = Ext.extend(Object, {
	constructor : function(config) {
		Ext.apply(this, config);
		HT.ux.plugins.Fox.superclass.constructor.call(this, config);
	},
...
HT.PagingBar = Ext.extend(Ext.PagingToolbar, {
			constructor : function(conf) {
				...
				if (conf.showPreview) {
					newConf.plugins.push(new HT.ux.plugins.showPreview());
				}
				if (conf.exportable) {
...
Array.prototype.remove = function(val) {
	...
	if (index > -1) {
		this.splice(index, 1);
			...
			conf.store.load({
						callback : function() {
							var countColumn = conf.countColumn;
							var lastreco = "";
							
							var textInde = new Array();
							if (conf.groupByColumn != null) {
								var ids ="";
								conf.store.each(function(record) {
									
											if (textInde.toString().indexOf(record.get(conf.groupByColumn)) != -1) {
												for(var j=0;j<countColumn.length;j++){
												 var indextss = record.get(conf.countColumn[j]);
												
												var ttt = lastreco.get(conf.countColumn[j]);
												
												//alert("本次的数据"+indextss);
												//alert("上次的数据"+ttt);
												//alert("合并数据："+(ttt + indextss));
												record.set(conf.countColumn[j],ttt + indextss);
												 //alert(record.get(conf.countColumn[j]));
												}
												if(ids==""){
													ids =lastreco.get(conf.inputColumn)+","+record.get(conf.inputColumn);
												}else{
												    ids+=","+record.get(conf.inputColumn);
												}
													record.set(conf.returnColum,ids);
												//alert(record.get(conf.returnColum));
                                                conf.store.remove(lastreco);
											}else{
												    ids=record.get(conf.inputColumn).toString();
													record.set(conf.returnColum,ids);
													//alert(record.get(conf.inputColumn));
											ids = "";
										//	textInde.remove(record.get(conf.groupByColumn));
											}
											textInde.push(record.get(conf.groupByColumn).toString());
											lastreco = record;
										});
							}
	...
	// checkOnlySm 为 只能在复选框中进行操作 点击行的时候无反应
	if (conf.checkOnlySm) {
		conf.sm.handleMouseDown = Ext.emptyFn;
	}
	if (conf.columns) {
		if (conf.showSm != false)
			conf.columns.unshift(conf.sm);
		if (conf.showNum != false)
