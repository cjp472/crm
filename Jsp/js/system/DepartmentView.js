...
DepartmentView.prototype.setup = function() {
					...
					renderer : function(appUser, metadata, record, rowIndex,
						...
						if (appUser) {
							...
							str += '&nbsp;<button title="添加上下级" value=" " class="btn-relativeJob" onclick="DepartmentView.addRelativeJob('
									...
									+ editdepId + '\')"></button>';
							// if (rowIndex > 0) {
							// str += '&nbsp;<button title="向上" value=" "
							// class="btn-up" onclick="DepartmentView.sn('
							//
							// + rowIndex + ',' + -1 + ')"></button>';
							// } else {
							// str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
							...
							// str += '&nbsp;<button title="向下" value=" "
							// class="btn-last" onclick="DepartmentView.sn('
							//
							// + rowIndex + ',' + 1 + ')"></button>';
							// }
	...
	function rowdblclickFn(grid, rowindex, e) {
		grid.getSelectionModel().each(function(rec) {
			DepartmentView.edit(rec.data.appUser.userId,
					rec.data.appUser.username);
		});
	}
	store.load({
				params : {
...
DepartmentView.add = function() {
	...
	var node = Ext.getCmp('departmentTreePanel').getSelectionModel()
			.getSelectedNode();
	/**
	 * 
	 * 此处修改成在员工列表添加员工方式
	 * 
	 * if(node != null && node.id > 0){ new DepUsersForm({ depId : node.id,
	 * depName : node.text }).show(); }else{ new DepUsersForm().show(); }
	 */
...
DepartmentView.edit = function(userId, userName) {// depUserId
	 ...
	 * 
	 * 
	 * if(node != null && node.id > 0){ new DepUsersForm({ depUserId :
	 * depUserId, depId : node.id, depName : node.text }).show(); }else{ new
	 * DepUsersForm({ depUserId : depUserId }).show(); }
	 */
	/**
