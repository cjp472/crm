...
var curDate = new Date().format('Y-m-d');
  var strDate=new Date().pattern("Y-m-d");
...
//									columnWidth : 1.1,
								...
								items : [{
										...
										UlEmployeeSelector.getView(function(userId,fullname,sex,useid,zhiwei,depName,depId, userNo,bankTypeId,bankname,username){			//UlEmployeeSelectors调用UlEmployeeSelectors这个JS
//													Ext.getCmp("userId").setValue(userId);						给隐藏域  用户ID赋值
													...
													Ext.getCmp("appUser.username").setValue(username);
													if(zhiwei !='null'){
													//Ext.getCmp("appUser.zhiwei").setValue(ZW001.get(zhiwei));
												      Ext.getCmp("appUser.zhiwei").setValue(zhiwei);
													}
...
//		},
		{
			...
			columnWidth : 0.5,
			title : '<font style="color:red">*</font>角色信息',
			collapsed : true,
			collapsible : true,
			autoHeight : true,
			defaults : {
