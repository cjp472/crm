 ...
 * @class ScBoPurchaseClassifyView
 * @extends Ext.Panel
 * @description [ScBoPurchaseClassify]管理
 * @company 优创融联科技
 * @createtime:
 */
ScBoPurchaseClassifyView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				ScBoPurchaseClassifyView.superclass.constructor.call(this, {
							id : 'ScBoPurchaseClassifyViewWin',
							title : '[ScBoPurchaseClassify]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				var ScBoPurchaseClassifyAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[ScBoPurchaseClassify]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'ScBoPurchaseClassifySearchPanel',
							height : 35,
													items:[
																						 																																												 										{
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new ScBoPurchaseClassifyAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[ScBoPurchaseClassify]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[ScBoPurchaseClassify]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'ScBoPurchaseClassifyGrid',
					url : __ctxPath + "/purchase/listScBoPurchaseClassify.do",
					fields : [{
									name : 'bizOrderDetailClassifyId',
									type : 'int'
								}
																																																																			,'scBoPurchaseClassify'
																																																																								,'desc'
																																																																								,'ext1'
																																																																								,'ext2'
																																																																								,'ext3'
...
//				var searchPanel = Ext.getCmp('ScBoPurchaseClassifySearchPanel');
...
//				}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new ScBoPurchaseClassifyForm({bizOrderDetailClassifyId:rec.data.bizOrderDetailClassifyId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new ScBoPurchaseClassifyForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ScBoPurchaseClassifyForm');
				if (aForm != null) {
					tabs.remove('ScBoPurchaseClassifyForm');
				}
				aForm = new ScBoPurchaseClassifyForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			removeRs : function(id) {
				$postSubmit({
					url:__ctxPath+ '/purchase/multiDelScBoPurchaseClassify.do',
					ids:id,
					grid:this.gridPanel,
					msgTip : '您确认要删除所选记录吗？',
					msgSuccess : '成功删除该记录！',
			...
			editRs : function(record) {
				...
				aForm = new ScBoPurchaseClassifyForm({bizOrderDetailClassifyId : record.data.bizOrderDetailClassifyId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
