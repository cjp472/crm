 ...
 * @class ScBizOrderPurchaseView
 * @extends Ext.Panel
 * @description [ScBizOrderPurchase]管理
 * @company 优创融联科技
 * @createtime:
 */
ScBizOrderPurchaseView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				ScBizOrderPurchaseView.superclass.constructor.call(this, {
							id : 'ScBizOrderPurchaseViewWin',
							title : '[ScBizOrderPurchase]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
							var fieldnameComboData=[
											...
											'createDeptId',
											'创建用户部门内码',
																								new Ext.form.TextField({name : 'createDeptId',allowBlank:true})
																						 ]
																				,
																			 								 																																		[
											'purUserId',
											'采购用户内码',
																																																					new Ext.form.NumberField({name : 'purUserId',allowBlank:true})
																																																	 ]
																				,
																			 								 																																		[
											'purDeptId',
											'采购用户部门内码',
																								new Ext.form.TextField({name : 'purDeptId',allowBlank:true})
																						 ]
																				,
																			 								 																																		[
											'custContPerson',
											...
											'折后小计',
																				...
																				,
																			 								 																																		[
											'purchaseModelType',
											'采购模式：0--买断模式，1--铺货代销模式&CON_T_PUR_MODEL',
																																																					new Ext.form.NumberField({name : 'purchaseModelType',allowBlank:true})
																																																	 ]
																				,
																			 								 																																		[
											'bizOrderStatus',
				...
				var ScBizOrderPurchaseAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[ScBizOrderPurchase]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'ScBizOrderPurchaseSearchPanel',
							height : 35,
													items:[
																						 																																												 										{
													...
													format:'Y-m-d'
																				...
																				,
																																				...
																																				hiddenName : 'Q_purchaseModelType_SN_EQ',
																																																																													xtype : 'mtdiccombo',
															editable : true,
															lazyInit : false,
															forceSelection : false,
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new ScBizOrderPurchaseAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[ScBizOrderPurchase]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[ScBizOrderPurchase]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'ScBizOrderPurchaseGrid',
					url : __ctxPath + "/purchase/listScBizOrderPurchase.do",
					fields : [{
									name : 'bizOrderId',
									type : 'int'
								}
										...
										return CON_T_BO_TYPE.get(value);
																...
																isExp : false,
																	...
																	dataIndex : 'discountAfterSubtotal'
																																																...
																																																,{
																	header : '采购模式：0--买断模式，1--铺货代销模式&CON_T_PUR_MODEL',
																isExp : false,
																
																	dataIndex : 'purchaseModelType',
									renderer : function(value) {
										return CON_T_PUR_MODEL.get(value);
									}
																}
																																																,{
...
//				var searchPanel = Ext.getCmp('ScBizOrderPurchaseSearchPanel');
...
//				}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new ScBizOrderPurchaseForm({bizOrderId:rec.data.bizOrderId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new ScBizOrderPurchaseForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ScBizOrderPurchaseForm');
				if (aForm != null) {
					tabs.remove('ScBizOrderPurchaseForm');
				}
				aForm = new ScBizOrderPurchaseForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			removeRs : function(id) {
				$postSubmit({
					url:__ctxPath+ '/purchase/multiDelScBizOrderPurchase.do',
					ids:id,
					grid:this.gridPanel,
					msgTip : '您确认要删除所选记录吗？',
					msgSuccess : '成功删除该记录！',
			...
			editRs : function(record) {
				...
				aForm = new ScBizOrderPurchaseForm({bizOrderId : record.data.bizOrderId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
