 ...
 * @class ScBizSalesDetailView
 * @extends Ext.Panel
 * @description [ScBizSalesDetail]管理
 * @company 优创融联科技
 * @createtime:
 */
ScBizSalesDetailView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				ScBizSalesDetailView.superclass.constructor.call(this, {
							id : 'ScBizSalesDetailViewWin',
							title : '[ScBizSalesDetail]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
							var fieldnameComboData=[
											...
											'goodsUnitPrice',
											'商品价格',
																																																					new Ext.form.NumberField({name : 'goodsUnitPrice',allowBlank:true})
																																																	 ]
																				,
																			 								 																																		[
											'goodsCount',
				...
				var ScBizSalesDetailAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[ScBizSalesDetail]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'ScBizSalesDetailSearchPanel',
							height : 35,
													items:[
																						 																																												 										{
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new ScBizSalesDetailAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[ScBizSalesDetail]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[ScBizSalesDetail]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'ScBizSalesDetailGrid',
					url : __ctxPath + "/sales/listScBizSalesDetail.do",
					fields : [{
...
//				var searchPanel = Ext.getCmp('ScBizSalesDetailSearchPanel');
...
//				}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new ScBizSalesDetailForm({bizOrderDetailId:rec.data.bizOrderDetailId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new ScBizSalesDetailForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ScBizSalesDetailForm');
				if (aForm != null) {
					tabs.remove('ScBizSalesDetailForm');
				}
				aForm = new ScBizSalesDetailForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			removeRs : function(id) {
				$postSubmit({
					url:__ctxPath+ '/sales/multiDelScBizSalesDetail.do',
					ids:id,
					grid:this.gridPanel,
					msgTip : '您确认要删除所选记录吗？',
					msgSuccess : '成功删除该记录！',
			...
			editRs : function(record) {
				...
				aForm = new ScBizSalesDetailForm({bizOrderDetailId : record.data.bizOrderDetailId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
