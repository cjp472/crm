...
SearchMail.prototype.grid = function(_searchContent) {
	var expander = new Ext.ux.grid.RowExpander({
		tpl : new Ext.Template('<p style="padding:5px 5px 5px 62px;"><b>内容摘要:</b> {content}</p>')
	...
	grid.addListener('rowdblclick', function(grid, rowIndex, e) {
				grid.getSelectionModel().each(function(rec) {
					SearchMail.detail(rec.data.mailId, rec.data.boxId,
							rec.data.mailStatus, rowIndex);
				});
			});
	return grid;
...
SearchMail.detail = function(mailId, boxId, status, rowIndex) {
	var detailToolbar = new SearchMail.centerViewToolbar(mailId, boxId,
			rowIndex);
	if (status == 0) {
	...
	var toolbar = new Ext.Toolbar({
				...
				items : [{
							...
							handler : function() {
								...
								var mailForm = Ext.getCmp('MailForm');
								if (mailForm == null) {
									mailForm = new MailForm(mailId, boxId,
											'reply');
									tab.add(mailForm);
									tab.activate(mailForm);
								} else {
