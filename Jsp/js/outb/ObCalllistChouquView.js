...
ObCalllistChouquView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ObCalllistChouquView.superclass.constructor.call(this, {
					id : 'ObCalllistChouquViewWin',
					title : '项目名单抽取',
					region : 'center',
					layout : 'border',
...
//									}),
		...
		this.searchPanel = new Ext.FormPanel({
					...
					items : [ {
                                ...
                                //name : 'Q_user.fullname_S_LK',
								name : 'Q_user.ulEmployee.userNo_S_LK',
								xtype : 'textfield'
							}, {
								xtype : 'panel',
								border : false,
		...
		this.gridPanel = new HT.GridPanel({
			...
			tbar : ['->',{
						...
						handler : function() {
							...
							var aForm = Ext.getCmp('ObCalllistChouquFormWin');
							if (aForm != null) {
								tabs.remove('ObCalllistChouquFormWin');
							}
							aForm = new ObCalllistChouquForm();
							tabs.add(aForm);
							tabs.activate(aForm);
						}
					}],
			...
			fields : [{
						name : 'extractId',
						type : 'Long'
					}, 'extractId','user','staDat','fromObCallbatch','toObCallbatch'],
			columns : [{
						header : 'extractId',
						dataIndex : 'extractId',
						hidden : true
						...
						renderer : function(val) {
							if(val.ulEmployee==null||val.ulEmployee=='undefined') {
							    return val.fullname;
							} else {
								return val.fullname+'('+val.ulEmployee.userNo+')';
							}							
							//return val.fullname+'('+val.ulEmployee.userNo+')';
						}
					}, {
					...
					},
					 {
						...
						dataIndex : 'fromObCallbatch',
						renderer : function(val) {
							return val.obCalllist.calllistNam;
						}
					}, {
						header : '来源批次',
						isExp : false,
	...
	// 重置查询表单
	reset : function() {
		Ext.getCmp('chouqu_staDat').setRawValue('');
...
//							}).show();
			var tabs = Ext.getCmp('centerTabPanel');
			var aForm = Ext.getCmp('ObConCalllistFormWin');
			if (aForm != null) {
				tabs.remove('ObConCalllistFormWin');
			}
			aForm = new ObConCalllistForm( {
				extractId : rec.data.extractId,
				toCallbatchId:rec.data.toObCallbatch.callbatchId,
				staDat:rec.data.staDat
			});
			tabs.add(aForm);
			tabs.activate(aForm);
		});
