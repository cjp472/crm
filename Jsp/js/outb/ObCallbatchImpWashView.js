 ...
 * @class ObCallbatchImpWashView
 * @extends Ext.Panel
 * @description [ObCallbatchImpWash]管理
 * @company 优创融联科技
 * @createtime:
 */
ObCallbatchImpWashView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				ObCallbatchImpWashView.superclass.constructor.call(this, {
							id : 'ObCallbatchImpWashViewWin',
							title : '[ObCallbatchImpWash]管理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				var ObCallbatchImpWashAdvancedSearchWin = Ext.extend(MT.AdvancedSearchWin, {
					title : '[ObCallbatchImpWash]高级查询',
				...
				this.searchPanel = new Ext.FormPanel({
							layout : 'hbox',
							region : 'north',
							id : 'ObCallbatchImpWashSearchPanel',
							height : 35,
													items:[
																						 																																												 										{
										...
										iconCls : 'search',
										scope : this,
										handler :function(){ new ObCallbatchImpWashAdvancedSearchWin().show();}
									}
								],
								layoutConfig : {
									padding : '5',
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									iconCls : 'btn-add',
									//text : __create+'[ObCallbatchImpWash]',
									text : __create,
									xtype : 'button',
									scope : this,
									...
									iconCls : 'btn-del',
									//text : __delete+'[ObCallbatchImpWash]',
									text : __delete,
									xtype : 'button',
									scope:this,
				...
				this.gridPanel=new HT.GridPanel({
					...
					printable : false,
					exportable : false,
					id:'ObCallbatchImpWashGrid',
					url : __ctxPath + "/outb/listObCallbatchImpWash.do",
					fields : [{
									name : 'washCusId',
									type : 'int'
								}
			...
			rowClick:function(grid,rowindex, e) {
				grid.getSelectionModel().each(function(rec) {
					new ObCallbatchImpWashForm({washCusId:rec.data.washCusId}).show();
				});
			},
			//创建记录
			createRs : function() {
				//new ObCallbatchImpWashForm().show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ObCallbatchImpWashForm');
				if (aForm != null) {
					tabs.remove('ObCallbatchImpWashForm');
				}
				aForm = new ObCallbatchImpWashForm();
				tabs.add(aForm);
				tabs.activate(aForm);
			...
			editRs : function(record) {
				//new ObCallbatchImpWashForm({
				//	washCusId : record.data.washCusId
				//}).show();
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ObCallbatchImpWashForm');
				if (aForm != null) {
				...
				aForm = new ObCallbatchImpWashForm({washCusId : record.data.washCusId});
				tabs.add(aForm);
				tabs.activate(aForm);
			},
			//行的Action
			onRowAction : function(grid, record, action, row, col) {
				switch (action) {
					case 'btn-del' :
						this.removeRs.call(this,record.data.washCusId);
						break;
					case 'btn-edit' :
