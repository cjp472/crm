 ...
 * @class ObCalllistZFeipeiView
 * @extends Ext.Panel
 * @description [ObConCalllist]管理
 * @company 优创融联科技
 * @createtime:
 */
ObCalllistZFeipeiView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ObCalllistZFeipeiView.superclass.constructor.call(this, {
					id : 'ObCalllistZFeipeiViewWin',
					title : '主管分配管理',
					region : 'center',
					layout : 'border',
...
//							}
		...
		this.topbar = new Ext.Toolbar({
					items : ['->', {
								...
								handler : function() {
									...
									var aForm = Ext
											.getCmp('ObCalllistJFeipeiFormWin');
									if (aForm != null) {
		...
		this.gridPanel = new HT.GridPanel({
			...
			exportable : false,
			//lazyLoad:true,
			id : 'ObCallbatchAssGridZZ',
			url : __ctxPath + "/outb/assign3ListObCallbatchAss.do?retriveIndex=1",
			fields : [{
						name : 'callbatchAssId',
						type : 'int'
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			...
			case 'btn-mail_move' :
				...
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ObCalllistZFeipeiFormWin');
				if (aForm != null) {
					tabs.remove('ObCalllistZFeipeiFormWin');
				}
				aForm = new ObCalllistZFeipeiForm({
