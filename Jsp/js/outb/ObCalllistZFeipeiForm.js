...
var val = 0;// 分配数量/比例
var assVal=0;//可分配数量
ObCalllistZFeipeiForm = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		ObCalllistZFeipeiForm.superclass.constructor.call(this, {
					id : 'ObCalllistZFeipeiFormWin',
					layout : 'fit',
					items : this.panel,
					modal : true,
					height : 400,
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.EditorGridPanel({
			...
			showPaging : false,
			url : __ctxPath
					+ "/outb/listManagersObCallbatchAss.do?type=2&comId="
					+ this.comId,
			fields : [{
						name : 'userId',
						type : 'int'
			...
			columns : [{
						...
						renderer : function(value, cls, record) {
							var re = /^[0-9]*[1-9][0-9]*$/;
							var holdVal = Ext.getCmp('ZZHoldCount').getValue();
							if (re.test(value)) {
								if (assignRuleType == 0) {
									if(holdVal * 1+ val * 1*holdVal/100 - value * 1*holdVal/100<0) {
									    ...
									    Ext.getCmp('ZZHoldCount').setValue(holdVal * 1
											+ val * 1*holdVal/100 - value * 1*holdVal/100);
										Ext.getCmp('assignTotalCountZZ').setValue(assVal*1-Ext.getCmp('ZZHoldCount').getValue()*1);	
										return value;	
									}									
								}
		...
		this.gridPanel_history = new HT.EditorGridPanel({
			...
			tbar : ['->', {
						...
						text : '增加',
						handler : function() {
							var store = Ext.getCmp('gridPanel_history_ZZ')
									.getStore();
						...
						handler : function() {
							var store = Ext.getCmp('gridPanel_history_ZZ')
									.getStore();
							var sm = Ext.getCmp('gridPanel_history_ZZ')
									.getSelectionModel();
			...
			columns : [{
				...
				dataIndex : 'assignIf',
				editor : new Ext.form.ComboBox({
							id : 'OB_CalllistZF_Grid_assignIf_01',
							mode : 'local',
							editable : false,
							triggerAction : 'all',
			...
			listeners : {
				'cellclick' : function(grid, rowIndex, columnIndex, e) {
					...
					if (columnIndex == 3) {
						var record = grid.getStore().getAt(rowIndex);
						if (record.data.assignFiled == '性别') {
							Ext.getCmp("OB_CalllistZF_Grid_assignIf_01")
									.getStore().loadData([['为空', '为空'],
											['等于', '等于'], ['不等于', '不等于']]);
						} else if (record.data.assignFiled == '证件类型') {
							Ext.getCmp("OB_CalllistZF_Grid_assignIf_01")
									.getStore().loadData([['为空', '为空'],
		...
		// collapsed : true,
		// collapsible : true,
		// items : [this.gridPanel_history_ZZ]
		// },{
		// xtype : 'fieldset',
		// collapsible : true,
		// title : '可分配人',
								...
								items : [{
									...
									listeners : {
										'check' : function(radio, check) {
											if (check) {
												assignRuleType = 0;
												Ext.get('fenpeiZZ').dom.style.display = 'block';
											}
										}
									}
								}, {
		...
		if (this.callbatchAssId != null && this.callbatchAssId != 'undefined') {
			Ext.getCmp('ZZHoldCount').setValue(this.holdCount);
			...
			this.formPanel.loadData({
						...
						success : function(response, options) {
							if (Ext.getCmp('ZZHoldCount').getValue() == 0) {
								Ext.getCmp("assignZZ_button").setVisible(false);
							}
						},
	...
	cancel : function() {
		...
		tabs.remove('ObCalllistZFeipeiFormWin');// 移除创建的窗口
	},
	/**
	 * 保存记录
	 */
	...
	assign : function() {
		...
		var assignTotalCount = Ext.getCmp('assignTotalCountZZ').getValue();
 		var re=/^[0-9]*[1-9][0-9]*$/;
 		if(!re.test(assignTotalCount)) {
		    Ext.ux.Toast.msg('操作信息', '分配数量格式不正确！');	
            return;
		    }		
		//var holdTotalCount = Ext.getCmp('ZZHoldCount').getValue();
		if (!assignTotalCount || assignTotalCount == 0) {
			Ext.ux.Toast.msg('操作提示', '分配数量不能为空或0!');
			return;
		}
		if (0 + assignTotalCount > assVal) {
		...
		Ext.Ajax.request({
					url : __ctxPath
							+ '/outb/doZZNewAssignCallbatchObCallbatchAss.do',
					method : 'post',
					params : {
						ids : idArray,
						counts : countArray,
					...
					success : function(response) {
						...
						if (result.success) {
							...
							var gridPanel = Ext.getCmp('ObCallbatchAssGridZZ');
							if (gridPanel != null) {
								gridPanel.getStore().reload();
							}
							var tabs = Ext.getCmp('centerTabPanel');
							tabs.remove('ObCalllistZFeipeiFormWin');
						}
					},
					failure : function() {
						myMask.hide();
