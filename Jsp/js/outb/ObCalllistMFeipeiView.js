 ...
 * @class ObCalllistMFeipeiView
 * @extends Ext.Panel
 * @description [ObConCalllist]管理
 * @company 优创融联科技
 * @createtime:
 */
ObCalllistMFeipeiView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ObCalllistMFeipeiView.superclass.constructor.call(this, {
					id : 'ObCalllistMFeipeiViewWin',
					title : '管理员名单分配',
					region : 'center',
					layout : 'border',
...
//							},
		...
		this.topbar = new Ext.Toolbar({
					items : [
							...
							// var aForm =
							// Ext.getCmp('ObCalllistMHuishouFormWin');
							// if (aForm != null) {
							...
							'->', {
								...
								handler : function() {
									var tabs = Ext.getCmp('centerTabPanel');
									var aForm = Ext
											.getCmp('ObCalllistMFeipeiFormWin');
									if (aForm != null) {
										tabs.remove('ObCalllistMFeipeiFormWin');
									}
									aForm = new ObCalllistMFeipeiForm();
									tabs.add(aForm);
									tabs.activate(aForm);
								}
							}]
...
//			countColumn : ['assignCount', 'retriveCount', 'holdCount','assignedCount','canReceiveCount'],
//			inputColumn : 'callbatchAssId',
//			returnColum : 'callbatchAssIds',

			id : 'ObCallbatchAssGridM',
			url : __ctxPath + "/outb/assign1ListObCallbatchAss.do?retriveIndex=0",
			fields : [{
						name : 'callbatchAssIds',
						type : 'String'
					}, 'callbatchAssIds','obCom','obCalllist','obCallbatch','assStepId','staDat','fromUseName','assignCount', 'holdCount','assignedCount','retriveCount','canReceiveCount','projNam'
					  //'obCallbatch','obCallbatch.callbatchNam','fromAppUser'
					],
			columns : [{
						header : 'callbatchAssIds',
						dataIndex : 'callbatchAssIds',
						hidden : true
					}, {
						header : '活动主题',
						isExp : false,
...
//						 renderer : function(value) {
//					 return CONOB_CON_CALLLIST_ZJLX
//					 .get(value);
//							if(value.ulEmployee!=null&&value.ulEmployee!=undefined) {
//							    return value.fromUseName+'('+value.ulEmployee.userNo+')';
//							} else {
//								return value.fromUseName;
//							}						 	
//					 }
				}	
//				, {
...
//					}
					, {
						...
						actions : [{
									...
									style : 'margin:0 3px 0 3px',
									fn : function(record) {
										var holdCount = record.get('canReceiveCount');
										if (holdCount == 0) {
											return false;
										} else {
											return true;
	...
	// 重置查询表单
	reset : function() {
		Ext.getCmp('MFP_assDat').setRawValue('');
	...
	setCallbatch : function(record) {
		Ext.Ajax.request({
			url : __ctxPath
					+ '/outb/listNewAssignCallbatchObCallbatchAss.do?callbatchId='
					+ record.data.obCallbatch.callbatchId,
			params : {},
			method : 'post',
			success : function(response) {
				...
				aForm = new ObCalllistMHuishouForm({
							callbatchAssIds : record.data.callbatchAssIds,
							assignCounts : record.data.assignCount,
							holdCounts : record.data.canReceiveCount,
							projNam:record.data.projNam,
							comNam:record.data.obCom.obComNam,
							calllistNam:record.data.obCalllist.calllistNam,
							callbatchNam:record.data.obCallbatch.callbatchNam,							
							retriveCounts : record.data.retriveCount,
							index:0,
							retriveIndex:0,  //管理员级别回收
							parentFlag:0//为0时为管理员分配页面HuishouForm
						});
				tabs.add(aForm);
				tabs.activate(aForm);
			}
	...
	onRowAction : function(grid, record, action, row, col) {
		switch (action) {
			case 'btn-mail_move' :
				...
				var tabs = Ext.getCmp('centerTabPanel');
				var aForm = Ext.getCmp('ObCalllistMFeipeiFormWin');
				if (aForm != null) {
					tabs.remove('ObCalllistMFeipeiFormWin');
				}
				aForm = new ObCalllistMFeipeiForm();
			...
			case 'btn-confApply-no' :
				this.setCallbatch.call(this, record);
				break;
