 ...
 * @class ObCalllistChouquUserView
 * @extends Ext.Panel
 * @description [ObConCalllist]管理
 * @company 优创融联科技
 * @createtime:
 */
ObCalllistChouquUserView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ObCalllistChouquUserView.superclass.constructor.call(this, {
					id : 'ObCalllistChouquUserViewWin',
					title : '客户名单抽取',
					region : 'center',
					layout : 'border',
		...
		this.gridPanel = new HT.GridPanel({
			...
			tbar : ['->',{
						...
						handler : function() {
							...
							var aForm = Ext.getCmp('ObCalllistChouquUserFormWin');
							if (aForm != null) {
								tabs.remove('ObCalllistChouquUserFormWin');
							}
							aForm = new ObCalllistChouquUserForm();
							tabs.add(aForm);
							tabs.activate(aForm);
						}
					}],
	...
	editRs : function(record) {
		...
		// }).show();
		var tabs = Ext.getCmp('centerTabPanel');
		var aForm = Ext.getCmp('ObConCalllistUserForm');
		if (aForm != null) {
			tabs.remove('ObConCalllistUserForm');
		}
		aForm = new ObConCalllistUserForm({
					cusId : record.data.cusId
				});
		tabs.add(aForm);
		tabs.activate(aForm);
