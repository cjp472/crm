...
public class ConBwlistApproveDaoImpl extends BaseDaoImpl<ConBwlistApprove> implements ConBwlistApproveDao{

	public ConBwlistApproveDaoImpl() {
		super(ConBwlistApprove.class);
	}
	...
	public void saveRunidAndNodeName(Long runId, String nodeName, Long headerId) {
		final String sql = "update con_bwlist_approve set runid=" + runId
		+ " ,node_Name='" + nodeName + "' where BWLIST_APPROVE_ID=" + headerId;
		getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {
				session.doWork(new Work() {
	...
	public void updateStatusByNodeName(String status, Long headerId,
		...
		final String sql = "update con_bwlist_approve set APPROVAL_STATUS='"
				+ status + "'" + tmp + "  where BWLIST_APPROVE_ID=" + headerId;
