...
public class InMessageDaoImpl extends BaseDaoImpl<InMessage> implements InMessageDao {
	...
	public InMessage findByRead(Long userId) {
		String hql="from InMessage vo where vo.readFlag=0 and vo.shortMessage.msgType=1 and vo.delFlag=0 and vo.userId=?";
		Object[] objs = {userId};
	...
	public Integer findByReadFlag(Long userId) {
		String sql="select count(*) from InMessage vo where vo.readFlag=0 and vo.delFlag=0 and vo.shortMessage.msgType=1 and vo.userId="+userId;
	...
	public List searchInMessageMonitor(Long userId) {
		StringBuffer hql=new StringBuffer("select vo1 ,vo2 from InMessage vo1,ShortMessage vo2 where vo1.shortMessage=vo2 and vo1.delFlag=0 and vo2.msgType=1 and vo2.senderId=");
		hql.append(userId);
		hql.append(" order by vo2.sendTime desc");
	...
	public List searchReceivedMsgInMessageMonitor(Long userId) {
		StringBuffer hql=new StringBuffer("select vo1 from InMessage vo1 where vo1.shortMessage.msgType=1 and vo1.delFlag=0 and vo1.userId=");
		hql.append(userId);
		hql.append(" order by vo1.shortMessage.sendTime desc");
