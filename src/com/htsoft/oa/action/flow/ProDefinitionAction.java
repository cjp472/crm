	...
	public String save() {
		...
		if (proDefinition.getDefId() != null) {// 更新操作
			...
			try {
				...
				proDef.setUpdateBy(ContextUtil.getCurrentUser());
				proDef.setUpdateTime(new Date());
				if ("true".equals(deploy)) {
					proDef.setStartBy(ContextUtil.getCurrentUser());
					proDef.setStartTime(new Date());
			...
			proDefinition.setCreatetime(new Date());
			proDefinition.setCreateBy(ContextUtil.getCurrentUser());
			if (logger.isDebugEnabled()) {
				logger.debug("---start deploy---");
			}

			if ("true".equals(deploy)) {
				proDefinition.setStartBy(ContextUtil.getCurrentUser());
				proDefinition.setStartTime(new Date());
...
//		
		...
		jsonString="{success:true,data:{decisionExpr:''}}";
		return SUCCESS;
	}
	/**
	 * 保存某个流程对应的某个节点的分支条件
...
//		filter.addParamValue(ProcessRun.RUN_STATUS_RUNNING);
        ...
        for(ProDefinition pd:list){
        	 ...
        	 .append("',updateTime:'").append(pd.getUpdateTime())
        	 .append("',createUserName:'").append(pd.getCreateBy().getFullname())
        	 .append("',updateUserName:'").append(pd.getUpdateBy().getFullname())
        	 .append("',startUserName:'").append(pd.getStartBy().getFullname())
        	 .append("',startTime:'").append(pd.getStartTime())
